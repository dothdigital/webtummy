...
function queueWebsiteWorkerErrorReport(job: { id: string; workspaceId: string; projectId: string; stage: string }, error: unknown) {
  const occurredAt = new Date();
  const errorCode = "SEN-WORKER-" + occurredAt.toISOString().slice(0, 10).replaceAll("-", "") + "-" + Math.random().toString(16).slice(2, 10).toUpperCase().padEnd(8, "0");
  const diagnostic = (error instanceof Error ? [error.name, error.message, error.stack].filter(Boolean).join("\n") : String(error || "No diagnostic detail was supplied.")).slice(0, 12_000);
  const context = ["Error code: " + errorCode, "Occurred: " + occurredAt.toISOString(), "Environment: " + (process.env.NODE_ENV || "development"), "Worker job: website-builder", "Job ID: " + job.id, "Stage: " + (job.stage || "Not identified"), "Project ID: " + job.projectId, "Workspace ID: " + job.workspaceId];
  void sendMail({
    to: config.supportEmail,
    subject: "[SEnuke AI Worker Error] " + errorCode + " · website-builder",
    text: ["SEnuke AI background worker error", "", ...context, "", "Sanitized diagnostic:", diagnostic].join("\n"),
    html: "<h2>SEnuke AI background worker error</h2><p>" + context.map(escapeHtml).join("<br>") + "</p><h3>Sanitized diagnostic</h3><pre style=\"white-space:pre-wrap\">" + escapeHtml(diagnostic) + "</pre><p>No request bodies, authorization headers, cookies, or credentials are included.</p>",
  }).catch((mailError) => {
    console.error("[worker] " + errorCode + ": support alert email failed", mailError instanceof Error ? mailError.message : mailError);
  });
}
    ...
    for (let index = 0; index < pages.length; index++) {
      ...
      if (contentModeForPage(page) === "targeted_update") {
        ...
        await prisma.$transaction(async (tx) => {
          ...
          if (executionTaskIds.length) await tx.seoFixQueueItem.updateMany({ where: { executionTaskId: { in: executionTaskIds } }, data: { approvalStatus: "content_ready_for_review" } });
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (executionTaskIds.length && !mediaOnlyRevision) {
          ...
          await tx.seoFixQueueItem.updateMany({ where: { executionTaskId: { in: executionTaskIds } }, data: { approvalStatus: completed ? "completed" : "content_ready_for_review" } });
        }
        if (visualPlan) {
