...
function TaskControl({ task, data, refresh }: { task: Task; data: Operations; refresh: () => Promise<void> }) {
  ...
  return <div className="rounded-lg border border-slate-200 p-4">
    ...
    {data.permissions.canAssignTasks && <div className="mt-4 grid gap-2 md:grid-cols-3 xl:grid-cols-6">
      ...
      <select value={form.approverMembershipId} onChange={(event) => setForm({ ...form, approverMembershipId: event.target.value })} className="h-9 rounded-lg border bg-white px-2 text-xs"><option value="">Manager/Approver</option>{data.members.filter((member) => member.roles.some((role) => ["manager", "approver", "admin", "owner"].includes(role.role))).map((member) => <option key={member.id} value={member.id}>{member.user.name || member.user.email}</option>)}</select>
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      ...
      {data.permissions.canPublish && task.status === "ready_to_publish" && <button disabled={Boolean(busy)} onClick={() => { if (window.confirm(`Publish the approved output for “${task.title}” to its configured destination?`)) void run("publish", () => api.post(`/api/execution-tasks/${task.id}/publish`, { confirmed: true })); }} className="rounded-lg bg-brand-600 px-3 py-2 text-xs font-bold text-white hover:bg-brand-700">Review & publish approved work</button>}
    </div>
