...
import ProjectVisitorSummary from "./ProjectVisitorSummary.js";
...
export default function ProjectWorkflowController({ projectId, websiteId, refreshKey = 0, compact = false, actionOnly = false, onLoaded, onNextAction, nextActionBusy = false, nextActionDisabled = false }: { projectId: string; websiteId?: string | null; refreshKey?: number | string; compact?: boolean; actionOnly?: boolean; onLoaded?: (workflow: Workflow) => void; onNextAction?: () => void; nextActionBusy?: boolean; nextActionDisabled?: boolean }) {
  ...
  if (!workflow && actionOnly) return <div role="status" className="text-xs text-slate-500">Loading next action…</div>;
  ...
  const actionControl = governanceAction ? <button type="button" onClick={() => void runNextAction()} disabled={actionBusy} className={`${actionOnly ? "sm:w-auto sm:shrink-0 !rounded-lg !px-3 !py-2 !text-xs" : ""} inline-flex w-full items-center justify-center rounded-xl border border-white/20 bg-gradient-to-r from-brand-700 to-violet-700 px-5 py-3 text-sm font-black text-white shadow-sm hover:from-brand-800 hover:to-violet-800 disabled:cursor-wait disabled:from-slate-400 disabled:to-slate-400`}>{actionBusy ? "Saving…" : next.action.label} <span className="ml-2">→</span></button> : onNextAction && next.title === "Create the Execution Plan" ? <button type="button" onClick={onNextAction} disabled={nextActionBusy || nextActionDisabled} className={`${actionOnly ? "sm:w-auto sm:shrink-0 !rounded-lg !px-3 !py-2 !text-xs" : ""} inline-flex w-full items-center justify-center rounded-xl border border-white/20 bg-gradient-to-r from-brand-700 to-violet-700 px-5 py-3 text-sm font-black text-white shadow-sm disabled:opacity-50`}>{nextActionBusy ? "Creating Execution Plan…" : next.action.label} <span className="ml-2">→</span></button> : <Link to={scopedUrl(next.action.url, projectId)} className={`${actionOnly ? "sm:w-auto sm:shrink-0 !rounded-lg !px-3 !py-2 !text-xs" : ""} inline-flex w-full items-center justify-center rounded-xl border border-white/20 bg-gradient-to-r from-brand-700 to-violet-700 px-5 py-3 text-sm font-black text-white shadow-sm hover:from-brand-800 hover:to-violet-800`}>{next.action.label} <span className="ml-2">→</span></Link>;
  if (actionOnly) return <div aria-label="Next execution action" className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
    <div className="min-w-0"><div className="text-[10px] font-bold uppercase tracking-wide text-brand-700">Next action</div><div className="break-words text-xs font-semibold text-slate-800">{next.title}</div></div>
    {actionControl}
  </div>;
  return <section className="overflow-hidden rounded-2xl border border-brand-200 bg-white shadow-sm" aria-label="Master project workflow">
    <div className="bg-gradient-to-r from-slate-950 via-brand-950 to-violet-950 px-5 py-5 text-white">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="max-w-3xl"><div className="text-[10px] font-black uppercase tracking-[0.22em] text-brand-200">AI Growth Operating System</div><h2 className="mt-1 text-xl font-black">Master Project Workflow</h2><p className="mt-2 text-sm leading-6 text-slate-200">AI guides your next action, from evidence to execution.</p><p className="mt-3 text-[10px] font-bold leading-5 text-brand-100">{canonicalLifecycle}</p></div>
        <div className="w-full min-w-0 space-y-3 xl:w-[390px] xl:shrink-0">
    ...
    <ProjectVisitorSummary key={websiteId ?? projectId} websiteId={websiteId} />
