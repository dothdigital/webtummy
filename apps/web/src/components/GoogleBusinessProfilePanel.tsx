...
export default function GoogleBusinessProfilePanel({ business, onBusinessRefresh, onMessage }: Props) {
  ...
  const providerProjectNumber = data?.providerProjectNumber ?? "the configured Google Cloud project";
        ...
        <div className="border-b border-slate-100 bg-[linear-gradient(110deg,#eef6ff_0%,#ffffff_48%,#f0fdf4_100%)] p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {quotaRequired && <Button onClick={() => void refreshLocations()} disabled={busy === "locations"}>{busy === "locations" ? "Checking access…" : "Check API access"}</Button>}
          ...
          {(message || data?.connection?.errorMessage) && <div className="mt-4 rounded-xl border border-blue-100 bg-white/80 px-4 py-3 text-sm text-charcoal-700">{message || data?.connection?.errorMessage}</div>}
          ...
          {quotaRequired && <div className="mt-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-4 text-sm leading-6 text-rose-950"><div className="font-black">Google account connected · Business Profile API access still required</div><p className="mt-1">Cloud project <b>{providerProjectNumber}</b> is returning zero/exhausted Account Management quota. Reconnecting the Google account will not fix this.</p><ol className="mt-2 list-decimal space-y-1 pl-5"><li>Submit Google’s <a className="font-black underline" href="https://support.google.com/business/contact/api_default" target="_blank" rel="noreferrer">Application for Basic API Access</a> for this exact project number.</li><li>After approval, enable the Business Profile APIs and confirm Account Management shows <b>300 requests/minute</b> rather than 0.</li><li>Return here and select <b>Check API access</b>.</li></ol><a className="mt-3 inline-flex font-black text-rose-800 underline" href={`https://console.cloud.google.com/apis/api/mybusinessaccountmanagement.googleapis.com/quotas${data?.providerProjectNumber ? `?project=${encodeURIComponent(data.providerProjectNumber)}` : ""}`} target="_blank" rel="noreferrer">Open Account Management quota in Google Cloud ↗</a></div>}
        </div>
