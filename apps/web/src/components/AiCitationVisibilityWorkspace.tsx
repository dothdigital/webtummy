...
const tabs: Array<{ key: Tab; label: string }> = [
  ...
  ...(ANSWER_OPPORTUNITIES_V2_ENABLED ? [{ key: "opportunities" as const, label: "Answer opportunities" }] : []),
  ...(AI_MONITORING_V2_ENABLED ? [{ key: "monitoring" as const, label: "AI monitoring" }] : []),
  ...(CITATION_RECOMMENDATIONS_V2_ENABLED ? [{ key: "recommendations" as const, label: "Recommendations" }] : []),
];
...
export default function AiCitationVisibilityWorkspace({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    setTab(initialTab);
    if (window.location.hash === "#business-facts" && initialTab === "entities") {
      window.requestAnimationFrame(() => document.getElementById("business-facts")?.scrollIntoView({ behavior: "smooth", block: "start" }));
    }
  }, [initialTab]);
  ...
  const audit = () => void run("audit", () => api.post(`/api/projects/${encodeURIComponent(projectId)}/ai-citation-visibility/audit`, {}), "Citation research completed. Evidence and readiness findings are ready for review.");
  const decideEntity = (id: string, decision: "approved" | "rejected") => void run(`entity:${id}`, () => api.patch(`/api/projects/${encodeURIComponent(projectId)}/ai-citation-visibility/entities/${encodeURIComponent(id)}`, { decision }), decision === "approved" ? "Entity approved for use in the SEO workflow." : "Entity rejected and excluded from the SEO workflow.");
  ...
  const editClaimWithAi = (claim: CitationWorkspace["claims"][number]) => {
    const instruction = window.prompt("What should AI change? The revised fact will still need your approval.", "Make this fact clearer without adding unsupported information.");
    if (!instruction?.trim()) return;
    void run(`claim-ai:${claim.id}`, () => api.post(`/api/projects/${encodeURIComponent(projectId)}/ai-citation-visibility/claims/${encodeURIComponent(claim.id)}/revise`, { instruction: instruction.trim() }), "AI created a revised fact. Review it before approving.");
  };
  ...
  const visibleRecommendations = useMemo(() => CITATION_RECOMMENDATIONS_V2_ENABLED
    ? workspace?.recommendations.filter((item) => ANSWER_OPPORTUNITIES_V2_ENABLED || item.recommendationType !== "answer_content") ?? []
    : [], [workspace]);
  const validatedAssetId = searchParams.get("generatedAssetId");
  const validatedAssetType = searchParams.get("generatedAssetType");

  if (!workspace && !error) return <Empty title="Loading citation intelligence…" detail="Assembling entity facts, crawl evidence, trust signals, and readiness findings." />;
    ...
    {!workspace ? <Empty title="Citation workspace unavailable" detail="The project data could not be loaded. Check the message above and try again." /> : <>
      <div className="overflow-hidden rounded-xl border border-brand-100 bg-white shadow-sm">
        <div className="flex flex-col gap-4 bg-gradient-to-r from-brand-50 via-white to-emerald-50 px-5 py-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            {workspace.audit && <div className="mt-2 text-xs font-semibold text-charcoal-400">Latest evidence snapshot: {formatDisplayDate(workspace.audit.createdAt)}</div>}
          </div>
        ...
        <div className={`grid grid-cols-2 divide-x divide-y divide-slate-100 ${(ANSWER_OPPORTUNITIES_V2_ENABLED || AI_MONITORING_V2_ENABLED) ? "sm:grid-cols-4" : "sm:grid-cols-2"}`}>
          ...
          {ANSWER_OPPORTUNITIES_V2_ENABLED && <div className="p-4 text-center"><div className="text-2xl font-black text-charcoal-950">{workspace.opportunities.filter((item) => item.status !== "superseded").length}</div><div className="text-[10px] font-black uppercase text-charcoal-400">Answer opportunities</div></div>}
          {AI_MONITORING_V2_ENABLED && <div className="p-4 text-center"><div className="text-2xl font-black text-charcoal-950">{latestObservations.length}</div><div className="text-[10px] font-black uppercase text-charcoal-400">Observed results</div></div>}
        </div>
      </div>
      ...
      {tab === "overview" && <div className="space-y-5">
        {!workspace.scores ? <Empty title="No citation research yet" detail="Run Citation Research to build an evidence snapshot from the project intake, approved keywords, latest crawl, and recorded AI observations." /> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          {AI_MONITORING_V2_ENABLED && <ScoreCard label="Observed AI visibility" value={workspace.scores.observedAiVisibility} helper="Measured only from saved observations." />}
        </div>}
      ...
      {tab === "entities" && <div id="business-facts" className="scroll-mt-24 space-y-5">
        {workspace.entities.map((entity) => <div key={entity.id} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-wide text-brand-700">{entity.entityType}</div><h3 className="mt-1 text-xl font-black text-charcoal-950">{entity.canonicalName}</h3><p className="mt-2 max-w-3xl text-sm leading-6 text-charcoal-600">{entity.description || "Business description requires review."}</p>{entity.canonicalUrl && <a href={entity.canonicalUrl} target="_blank" rel="noreferrer" className="mt-2 block text-xs font-bold text-brand-700">{entity.canonicalUrl}</a>}</div><div className="text-right"><Pill value={entity.verificationStatus} /><div className="mt-2 text-xs font-bold text-charcoal-400">{entity.confidence}% confidence</div>{workspace.capabilities.canApprove && !["approved", "rejected"].includes(entity.verificationStatus) && <div className="mt-3 flex gap-2"><button type="button" onClick={() => decideEntity(entity.id, "rejected")} disabled={Boolean(busy)} className="rounded-lg border border-rose-200 px-3 py-2 text-xs font-black text-rose-700">Reject</button><button type="button" onClick={() => decideEntity(entity.id, "approved")} disabled={Boolean(busy)} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-black text-white">{busy === `entity:${entity.id}` ? "Saving…" : "Approve"}</button></div>}</div></div></div>)}
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 px-5 py-4"><h3 className="font-black text-charcoal-950">Claim register</h3><p className="mt-1 text-sm text-charcoal-500">Each fact is classified by origin and tied to its source. Approve facts before using them in generated content or schema.</p></div>{workspace.claims.length ? <div className="divide-y divide-slate-100">{workspace.claims.map((claim) => <div key={claim.id} className="p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Pill value={claim.classification} /><Pill value={claim.verificationStatus} /><span className="text-xs font-bold text-charcoal-400">{claim.confidence}% confidence</span></div><div className="mt-3 text-xs font-black uppercase tracking-wide text-charcoal-400">{display(claim.claimType)}</div><p className="mt-1 text-sm font-semibold leading-6 text-charcoal-900">{claim.statement}</p><div className="mt-2 text-xs text-charcoal-500">Source: {claim.sources.map((source) => source.sourceLabel).join(", ") || "No source recorded"}</div></div>{workspace.capabilities.canApprove && !["approved", "rejected"].includes(claim.verificationStatus) && <div className="flex shrink-0 flex-wrap gap-2"><button type="button" onClick={() => editClaimWithAi(claim)} disabled={Boolean(busy)} className="rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-xs font-black text-violet-700">{busy === `claim-ai:${claim.id}` ? "Editing…" : "Edit with AI"}</button><button type="button" onClick={() => decideClaim(claim.id, "rejected")} disabled={Boolean(busy)} className="rounded-lg border border-rose-200 px-3 py-2 text-xs font-black text-rose-700">Reject</button><button type="button" onClick={() => decideClaim(claim.id, "approved")} disabled={Boolean(busy)} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-black text-white">{busy === `claim:${claim.id}` ? "Saving…" : "Approve fact"}</button></div>}</div></div>)}</div> : <Empty title="No claims recorded" detail="Run citation research to create a source-backed claim register from existing project data." />}</div>
      </div>}
      ...
      {AI_MONITORING_V2_ENABLED && tab === "monitoring" && <div className="space-y-5">
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 px-5 py-4"><h3 className="font-black text-charcoal-950">Observation history</h3></div>{latestObservations.length ? <div className="divide-y divide-slate-100">{latestObservations.map((item) => <div key={item.id} className="p-5"><div className="flex flex-wrap items-center gap-2"><Pill value={item.visibilityStatus} />{item.accuracyStatus && <Pill value={item.accuracyStatus} />}<span className="text-xs text-charcoal-400">{item.scanProvider} · {formatDisplayDate(item.createdAt)}</span></div><div className="mt-2 text-sm font-black text-charcoal-900">{item.prompt}</div>{item.answerExcerpt && <p className="mt-2 rounded-lg bg-slate-50 p-3 text-sm leading-6 text-charcoal-600">{item.answerExcerpt}</p>}{item.sourceMentions.length > 0 && <div className="mt-2 space-y-1">{item.sourceMentions.map((source) => <a key={source.id} href={source.sourceUrl} target="_blank" rel="noreferrer" className="block break-all text-xs font-semibold text-brand-700">{source.sourceDomain} · {source.sourceUrl}</a>)}</div>}</div>)}</div> : <Empty title="No observed results" detail="Visibility stays unassessed until a real provider result or documented manual check is recorded." />}</div>
      </div>}

      {CITATION_RECOMMENDATIONS_V2_ENABLED && tab === "recommendations" && <div className="space-y-3">{visibleRecommendations.length ? visibleRecommendations.map((recommendation) => {
