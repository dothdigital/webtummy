...
type CitationWorkspace = {
  ...
  findings: Array<{ id: string; category: string; title: string; summary: string; severity: string; confidence: number; scoreImpact: number; evidenceJson: unknown; isInference: boolean; recommendedAction: string; status: string }>;
...
export default function AiCitationVisibilityWorkspace({ projectId }: { projectId: string }) {
  ...
  return <div className="space-y-5">
    ...
    {!workspace ? <Empty title="Citation workspace unavailable" detail="The project data could not be loaded. Check the message above and try again." /> : <>
      ...
      {tab === "recommendations" && <div className="space-y-3">{workspace.recommendations.length ? workspace.recommendations.map((recommendation) => <div key={recommendation.id} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Pill value={recommendation.status} /><Pill value={recommendation.recommendationType} /><span className="text-xs font-bold text-charcoal-400">Priority {recommendation.priorityScore} · {display(recommendation.riskLevel)} risk</span></div><h3 className="mt-3 text-lg font-black text-charcoal-950">{recommendation.title}</h3><p className="mt-1 text-sm leading-6 text-charcoal-600">{recommendation.rationale}</p><div className="mt-3 rounded-lg border border-brand-100 bg-brand-50 p-3 text-sm leading-6 text-brand-900"><b>Recommended action:</b> {recommendation.recommendedAction}</div>{Object.keys(record(recommendation.contentDraftJson)).length > 0 && <details className="mt-3 rounded-lg border border-slate-200 p-3"><summary className="cursor-pointer text-xs font-black text-charcoal-700">Review content brief</summary><pre className="mt-3 overflow-x-auto whitespace-pre-wrap text-xs leading-5 text-charcoal-600">{JSON.stringify(recommendation.contentDraftJson, null, 2)}</pre></details>}{Object.keys(record(recommendation.schemaDraftJson)).length > 0 && <details className="mt-3 rounded-lg border border-slate-200 p-3"><summary className="cursor-pointer text-xs font-black text-charcoal-700">Review schema draft</summary><pre className="mt-3 overflow-x-auto whitespace-pre-wrap text-xs leading-5 text-charcoal-600">{JSON.stringify(recommendation.schemaDraftJson, null, 2)}</pre></details>}</div>{workspace.capabilities.canApprove && recommendation.status === "proposed" && <button type="button" onClick={() => approveRecommendation(recommendation.id)} disabled={Boolean(busy)} className="shrink-0 rounded-lg bg-emerald-600 px-4 py-2 text-xs font-black text-white">{busy === `recommendation:${recommendation.id}` ? "Approving…" : "Approve & create task"}</button>}</div></div>) : <Empty title="No proposed recommendations" detail="Run citation research to generate evidence-backed content, schema, trust, and correction recommendations." />}</div>}
    </>}
  </div>;
}
