...
// Phase 2: restore this after crawler/Website Model question matching, FAQ and
// schema insertion, publication URL capture, and monitoring are one workflow.
...
export default function AiCitationVisibilityWorkspace({ projectId }: { projectId: string }) {
  ...
  const audit = () => void run("audit", () => api.post(`/api/projects/${encodeURIComponent(projectId)}/ai-citation-visibility/audit`, {}), "Citation research completed. Evidence and proposed recommendations are ready for review.");
  ...
  const visibleRecommendations = useMemo(() => workspace?.recommendations.filter((item) => ANSWER_OPPORTUNITIES_V2_ENABLED || item.recommendationType !== "answer_content") ?? [], [workspace]);
  const validatedAssetId = searchParams.get("generatedAssetId");
  const validatedAssetType = searchParams.get("generatedAssetType");

  if (!workspace && !error) return <Empty title="Loading citation intelligence…" detail="Assembling entity facts, crawl evidence, trust signals, and observed AI visibility." />;
  ...
  <div className="space-y-5">
    ...
    {!workspace ? <Empty title="Citation workspace unavailable" detail="The project data could not be loaded. Check the message above and try again." /> : <>
      ...
      {tab === "recommendations" && <div className="space-y-3">{visibleRecommendations.length ? visibleRecommendations.map((recommendation) => {
