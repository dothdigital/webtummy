...
export default function SiteCapabilityExtension({ projectId, crawlCompletedAt }: { projectId: string; crawlCompletedAt?: string | null }) {
  ...
  const priorityGaps = useMemo(() => (run?.results ?? [])
    .filter((item) => ["BLOCKED", "MISSING", "PARTIAL"].includes(item.status))
    .sort((left, right) => ({ BLOCKED: 0, MISSING: 1, PARTIAL: 2 }[left.status] ?? 3) - ({ BLOCKED: 0, MISSING: 1, PARTIAL: 2 }[right.status] ?? 3))
    .slice(0, 6), [run]);
  ...
  return <div className="border-t border-slate-100">
    ...
    <div className="border-t border-slate-100 bg-slate-50/60 px-5 py-4">
      ...
      <div className="mt-3 grid gap-2 lg:grid-cols-2">
        {priorityGaps.map((item) => <div key={item.capabilityId} className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-3 py-3"><div className="min-w-0"><div className="flex items-center gap-2"><span className={`rounded-full px-2 py-0.5 text-[9px] font-black uppercase ${statusStyle[item.status]}`}>{item.status.toLowerCase()}</span><span className="truncate text-sm font-black text-slate-900">{item.title}</span></div><p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-500">{item.message}</p></div><Link to={actionUrl(item, projectId)} className="shrink-0 rounded-lg bg-slate-950 px-3 py-2 text-xs font-black text-white">Resolve</Link></div>)}
        {!priorityGaps.length && <div className="text-sm font-semibold text-emerald-700">No missing, blocked, or partial connected checks remain.</div>}
      </div>
    </div>
  </div>;
}
