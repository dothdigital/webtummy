...
import type { GuidedExecutionTask, GuidedProject, ProjectNotification, ProjectWorkflowController } from "../types.js";
import { api } from "../api.js";
import ExecutionTaskBrief, { executionTaskBrief } from "./ExecutionTaskBrief.js";
...
export default function ProjectExecutionBar({ project, tasks: suppliedTasks, notifications = [] }: { project: GuidedProject; tasks?: GuidedExecutionTask[]; notifications?: ProjectNotification[] }) {
  ...
  const [workflow, setWorkflow] = useState<ProjectWorkflowController | null>(null);
  ...
  const collectedTasks = Array.from([...(suppliedTasks ?? planTasks), ...inboxTasks].reduce((map, rawTask) => {
  ...
  const seoPlanStage = workflow?.stages.find((stage) => stage.key === "seo_plan");
  // Stored plans remain available for history, but they must not compete with
  // the controller's required Growth step or appear actionable before unlock.
  // Fail closed while the controller is loading or if the stage is blocked.
  const tasks = collectedTasks.filter((task) => !isContentPlanTask(task) || Boolean(seoPlanStage && seoPlanStage.status !== "blocked"));
  ...
  useEffect(() => {
    let active = true;
    const loadWorkflow = () => api.get<{ workflow: ProjectWorkflowController }>(`/api/projects-v2/${encodeURIComponent(project.id)}/workflow-controller`)
      .then((result) => { if (active) setWorkflow(result.workflow); })
      .catch(() => { if (active) setWorkflow(null); });
    void loadWorkflow();
    const refresh = () => { void loadWorkflow(); };
    const timer = window.setInterval(refresh, 8000);
    window.addEventListener("senuke:workflow-refresh", refresh);
    window.addEventListener("focus", refresh);
    return () => { active = false; window.clearInterval(timer); window.removeEventListener("senuke:workflow-refresh", refresh); window.removeEventListener("focus", refresh); };
  }, [project.id, taskStatusKey]);
  useEffect(() => {
  ...
  const seoGapExecutionReady = tasks.some((task) => task.moduleName === "gap_analysis" && !/(?:run seo|update strategy|approve strategy)/i.test(`${task.title} ${task.actionButtonLabel ?? ""}`));
  ...
  const openContentPlan = async (task: GuidedExecutionTask) => {
    ...
    } catch { /* Open the saved task when task preparation is temporarily unavailable. */ }
  ...
  const officialNextAction = workflow?.nextBestAction ?? null;
  const officialNextHref = officialNextAction ? projectScopedHref(officialNextAction.action.url, project.id) : null;
  ...
  return <>
    {officialNextAction && officialNextHref
      ? <div className="inline-flex flex-wrap items-center gap-2"><Link to={officialNextHref} className="inline-flex items-center gap-2 rounded-md px-1.5 py-1 transition hover:bg-slate-50" title={officialNextAction.explainability}>{loadingInbox ? <span className="text-xs font-semibold text-charcoal-500">Loading project actions…</span> : tracker}<span className="text-xs font-bold text-brand-700">{officialNextAction.action.label} →</span></Link>{pending.length > 0 && <button type="button" onClick={() => { setFilter("pending"); setOpen(true); }} className="text-xs font-bold text-charcoal-500 hover:text-brand-700">Tasks ({pending.length})</button>}</div>
      : <span className="inline-flex items-center gap-2 rounded-md px-1.5 py-1 text-xs font-semibold text-charcoal-500">{loadingInbox ? "Loading project actions…" : tracker}<span>Loading next action…</span></span>}
      ...
      <aside className="ml-auto flex h-full w-full max-w-2xl flex-col bg-white shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {filter === "all" && sourceSummaries.length > 0 && <div className="mt-5 space-y-3">
            {sourceSummaries.filter((summary) => ["keyword_research", "domain", "site_architect"].includes(summary.key) || (summary.key === "gap_analysis" && seoGapExecutionReady)).map((summary) => <div key={summary.key} className="rounded-xl border border-slate-200 bg-white p-4"><div className="flex items-center justify-between gap-3"><div><div className="font-bold text-charcoal-900">{summary.label}</div><div className="mt-0.5 text-xs text-charcoal-500">{summary.key === "gap_analysis" ? "Consolidated executable actions; open the plan to drill into exact source findings." : "Combined source activity behind the Execution Plan"}</div></div><Link to={projectScopedHref(summary.actionUrl, project.id)!} onClick={() => setOpen(false)} className="shrink-0 text-xs font-bold text-brand-700">{summary.key === "gap_analysis" ? "Review plan" : `Review all ${summary.total}`} →</Link></div><div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-5">{summary.metrics.map((metric) => <div key={metric.label} className="rounded-md border border-slate-200 bg-slate-50 px-2.5 py-2"><div className="text-[9px] font-black uppercase tracking-wide text-charcoal-400">{metric.label}</div><div className="mt-0.5 text-lg font-black text-charcoal-800">{metric.value}</div></div>)}</div></div>)}
          </div>}
