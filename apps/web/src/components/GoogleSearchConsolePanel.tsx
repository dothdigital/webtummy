...
export default function GoogleSearchConsolePanel({ projectId }: { projectId: string }) {
  ...
  return <section id="search-performance" className="scroll-mt-6 overflow-hidden rounded-2xl border border-indigo-200 bg-white">
    ...
    <div className="space-y-4 p-5">
      ...
      {connection?.propertyUrl && <div className="flex flex-wrap justify-between gap-2 text-xs text-slate-600"><b className="break-all">{connection.propertyUrl}</b><span>{connection.lastSyncedAt ? `Last synced ${formatDisplayDate(connection.lastSyncedAt)}` : "Waiting for the first successful import"}</span></div>}
