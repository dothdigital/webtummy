...
export default function ConversationalProjectIntake(props: Props) {
  ...
  async function researchProject(instruction?: string) {
    ...
    try {
      ...
      setError(requestError instanceof DOMException && requestError.name === "TimeoutError" ? "The research is taking longer than expected. Your saved project is safe—please run it again." : requestError instanceof Error ? requestError.message : "SEnuke AI could not complete the project research.");
  ...
  return <form onSubmit={props.onCreate} className="space-y-4">
    ...
    <div className="grid gap-5 xl:h-[calc(100dvh-175px)] xl:min-h-[650px] xl:max-h-[920px] xl:grid-cols-[minmax(0,1fr)_410px]">
      ...
      </div><div className="border-t bg-slate-50 p-4">{readyForReview ? <><div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3"><div className="text-xs font-black text-emerald-950">Your project foundation is ready for research</div><p className="mt-1 text-[11px] leading-5 text-emerald-800">SEnuke AI will now evaluate the complete intake and prepare a professional proposal before anything is finalized.</p></div><Button type="button" onClick={() => void researchProject()} disabled={props.busy || researching} className="mt-3 w-full">Research My Idea & Build Proposal →</Button><button type="button" onClick={() => void send("Before research, show me a final summary and tell me what could still improve the proposal.")} className="mt-2 w-full rounded-lg border bg-white px-3 py-2 text-xs font-bold text-slate-700">Review the intake once more</button></> : <div className="rounded-xl border border-amber-200 bg-amber-50 p-3"><div className="flex items-start gap-3"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-amber-100 text-sm">🔒</span><div><div className="text-xs font-black text-amber-950">Complete the essential intake first</div><p className="mt-1 text-[11px] leading-5 text-amber-800">Continue answering the current required question. Once the essential project information is captured, SEnuke AI can research and prepare the complete proposal.</p></div></div></div>}</div></Card>
    </div>
  </form>;
}
