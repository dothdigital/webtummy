  ...
  return <div className="space-y-6 pb-10">
    <section className="overflow-hidden rounded-2xl bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 p-6 text-white shadow-xl">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Link to={`/site-analysis?projectId=${projectId}`} className="rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-sm font-black text-white hover:bg-white/15">Website Intelligence</Link><Button disabled={!data?.project.ecommerceContext || !data?.capabilities.canAnalyze || busy === "analyze"} onClick={() => void runAnalysis()}>{busy === "analyze" ? "Analyzing…" : data?.latestSavedRun ? "Refresh Ecommerce Intelligence" : "Run Ecommerce Intelligence"}</Button></div>
      </div>
    </section>
    ...
    {data && !data.project.ecommerceContext && <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4"><div><div className="font-black text-amber-950">Ecommerce Business Type required</div><p className="mt-1 text-sm text-amber-900">This module is shared across all workspaces, but it analyzes only projects identified as ecommerce stores.</p></div><Link to={`/projects/new?edit=${projectId}`} className="rounded-lg bg-amber-900 px-4 py-2.5 text-sm font-black text-white">Edit Business Type</Link></div>}

    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      {[
        ["Products detected", intelligence?.store.productCount ?? 0, "Public crawl"], ["Collections detected", intelligence?.store.collectionCount ?? 0, "Public crawl"], ["Buying guides", intelligence?.store.guideCount ?? 0, "Supporting content"], ["Recommendations", intelligence?.recommendations.length ?? 0, "Strategy candidates"], ["Performance evidence", data?.readiness.performanceEvidence ? "Supplied" : "Unknown", data?.readiness.performanceEvidence ? "User-provided" : "Never treated as zero"],
    ...
    {tab === "overview" && <div className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
      ...
      ].map(([number, title, body]) => <div key={number} className="rounded-xl border border-slate-200 p-4"><span className="grid h-8 w-8 place-items-center rounded-lg bg-slate-950 text-xs font-black text-white">{number}</span><h3 className="mt-3 font-black text-slate-950">{title}</h3><p className="mt-1 text-xs leading-5 text-slate-600">{body}</p></div>)}</div><div className="mt-5 rounded-xl bg-emerald-50 p-4 text-sm leading-6 text-emerald-950"><b>Workspaces determine context, not capability.</b> Ecommerce projects retain Website Development, SEO, Local SEO, AI Citations, Authority, Content, Lead Magnets, Social, Publishing, Measurement, Growth Blueprint, and Next Best Action.</div></Card>
      <Card className="p-5"><div className="text-xs font-black uppercase tracking-wide text-slate-400">Readiness</div><div className="mt-4 space-y-3">{Object.entries(data?.readiness ?? {}).map(([key, value]) => <div key={key} className="flex items-center justify-between gap-4 border-b border-slate-100 pb-3 text-sm"><span className="font-bold text-slate-700">{label(key)}</span><span className={`rounded-full px-2.5 py-1 text-xs font-black ${value === true || value === "approved" ? "bg-emerald-100 text-emerald-800" : value === false || value === "not_started" ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-700"}`}>{typeof value === "boolean" ? value ? "Ready" : "Needed" : label(String(value))}</span></div>)}</div>{!data?.readiness.publicCrawl && <Link to={`/site-analysis?projectId=${projectId}`} className="mt-4 inline-flex rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-black text-white">Analyze Public Store</Link>}</Card>
    </div>}
