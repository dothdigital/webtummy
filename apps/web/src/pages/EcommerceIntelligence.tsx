...
export default function EcommerceIntelligence() {
  ...
  const approve = async (key: string) => {
    ...
    try {
      const result = await api.post<{ message: string; nextAction: { label: string; url: string } }>(`/api/projects/${projectId}/ecommerce-intelligence/recommendations/${encodeURIComponent(key)}/approve`, {});
      await load();
  ...
  return <div className="space-y-6 pb-10">
    <section className="overflow-hidden rounded-2xl bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 p-6 text-white shadow-xl">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Button disabled={!data?.project.ecommerceContext || !data?.capabilities.canAnalyze || busy === "analyze"} onClick={() => void runAnalysis()}>{busy === "analyze" ? "Researching…" : data?.latestSavedRun ? "Refresh Ecommerce Research" : data?.readiness.storeUrl ? "Run Ecommerce Intelligence" : "Start Ecommerce Research"}</Button>{!data?.readiness.storeUrl && <Link to={editStoreUrl} className="rounded-xl border border-white/30 px-4 py-3 text-sm font-black text-white">Add Store URL Later</Link>}</div>
      </div>
    </section>
    ...
    {tab === "recommendations" && <div className="space-y-4">{intelligence?.recommendations.map((item, index) => { const approved = data?.approvedRecommendationKeys.includes(item.key); return <Card key={item.key} className={`p-5 ${approved ? "border-emerald-300 bg-emerald-50/30" : ""}`}><div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between"><div className="max-w-4xl"><div className="flex flex-wrap items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-lg bg-slate-950 text-xs font-black text-white">{index + 1}</span><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${priorityTone(item.priority)}`}>{item.priority}</span><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${evidenceTone(item.evidenceType)}`}>{label(item.evidenceType)} evidence</span><span className="rounded-full bg-violet-100 px-2.5 py-1 text-[10px] font-black uppercase text-violet-800">{label(item.destination)}</span>{approved && <span className="rounded-full bg-emerald-600 px-2.5 py-1 text-[10px] font-black uppercase text-white">✓ Approved for Strategy</span>}</div><h2 className="mt-3 text-lg font-black text-slate-950">{item.title}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{item.explanation}</p><div className="mt-4 rounded-xl border border-brand-100 bg-brand-50/60 p-4"><div className="text-[10px] font-black uppercase tracking-wide text-brand-700">Recommended action</div><p className="mt-1 text-sm font-semibold leading-6 text-brand-950">{item.recommendedAction}</p></div><div className="mt-3 text-xs font-bold text-emerald-800"><span className="font-black">Expected direction:</span> {item.expectedImpact}</div></div><div className="min-w-[220px] rounded-xl bg-slate-50 p-4"><div className="grid grid-cols-2 gap-3"><div><div className="text-[10px] font-black uppercase text-slate-400">Impact</div><div className="text-2xl font-black text-slate-950">{item.impactScore}</div></div><div><div className="text-[10px] font-black uppercase text-slate-400">Confidence</div><div className="text-2xl font-black text-slate-950">{item.confidenceScore}%</div></div></div>{data?.capabilities.canApprove && (approved ? <><div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2.5 text-center text-xs font-black text-emerald-800">✓ Saved and approved</div><Link to={`/strategy?projectId=${encodeURIComponent(projectId)}`} className="mt-2 block rounded-lg bg-slate-950 px-4 py-2.5 text-center text-xs font-black text-white">Review updated Strategy →</Link></> : <button disabled={busy === `approve:${item.key}`} onClick={() => void approve(item.key)} className="mt-4 w-full rounded-lg bg-brand-600 px-4 py-2.5 text-xs font-black text-white disabled:bg-slate-300">{busy === `approve:${item.key}` ? "Approving…" : "Approve for Strategy"}</button>)}<details className="mt-3"><summary className="cursor-pointer text-xs font-black text-slate-600">View evidence</summary><ul className="mt-2 space-y-1 text-[11px] leading-5 text-slate-500">{item.evidence.map((evidence) => <li key={evidence}>• {evidence}</li>)}</ul></details></div></div></Card>})}{!intelligence?.recommendations.length && <EmptyState title="Run Ecommerce Intelligence" description={data?.readiness.storeUrl ? "Complete the public crawl, then generate the product and collection opportunity report." : "Add confirmed products in Business Discovery, then start pre-launch Ecommerce Research."} />}</div>}
