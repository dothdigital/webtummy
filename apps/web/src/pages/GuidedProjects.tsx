...
function WorkflowMilestones({ project }: { project: GuidedProject }) {
  ...
  return <div className="mt-5"><div className="mb-3 flex items-center justify-between gap-3"><div className="text-xs font-bold uppercase tracking-wide text-slate-500">Milestones</div><div className="text-right text-[11px] text-slate-400">{completed} achieved · {skipped} skipped · {steps.length} total</div></div><div className="relative w-full pt-1"><div className="absolute left-3 right-3 top-3.5 h-0.5 bg-slate-200"><div className="h-full bg-emerald-400" style={{ width: `${lineProgress}%` }} /></div><div className="relative flex w-full justify-between">{steps.map((step, index) => { const state = workflowState(project, index); const edgeAlign = index === 0 ? "items-start text-left" : index === steps.length - 1 ? "items-end text-right" : "items-center text-center"; return <div key={step.id} className={`flex min-w-0 flex-1 flex-col ${edgeAlign}`}><span className={`relative z-[1] flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[9px] font-black shadow-[0_0_0_3px_white] ${state === "completed" ? "bg-emerald-500 text-white" : state === "skipped" ? "bg-slate-300 text-slate-700" : state === "current" ? "bg-teal-500 text-white ring-4 ring-teal-100" : state === "review" ? "bg-amber-500 text-white" : state === "blocked" ? "bg-rose-500 text-white" : "border-2 border-slate-300 bg-white text-slate-400"}`}>{state === "completed" ? "✓" : state === "skipped" ? "–" : state === "current" ? "●" : state === "review" ? "!" : state === "blocked" ? "×" : index + 1}</span><div className={`mt-2 max-w-[104px] text-[10px] font-bold leading-3.5 ${state === "completed" ? "text-emerald-700" : state === "current" ? "text-teal-800" : state === "review" ? "text-amber-800" : state === "blocked" ? "text-rose-700" : "text-slate-500"}`}>{step.title}</div><div className="mt-0.5 text-[8px] font-bold uppercase tracking-wide text-slate-400">{state === "completed" ? "Achieved" : state === "review" ? "Review" : state}</div></div>; })}</div></div></div>;
}
