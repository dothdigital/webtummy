...
export default function Approvals() {
  ...
  const load = async (id = projectId) => {
    ...
    const result = await api.get<{ tasks: ApprovalTask[] }>(`/api/approvals${suffix}`);
  ...
  useEffect(() => {
    api.get<{ projects: Project[] }>("/api/projects-v2").then((result) => {
      ...
      if (!projectId && result.projects[0]) setProjectId(result.projects[0].id);
    }).catch(() => setProjects([]));
  }, []);
  useEffect(() => {
    if (!projectId) return;
    ...
    void Promise.all([
      load(projectId),
      api.get<{ automationLevel: AutomationLevel }>(`/api/projects-v2/${projectId}/approval-policy`).then((result) => setLevel(result.automationLevel)),
    ]).catch((error) => setMessage(error instanceof Error ? error.message : "Approvals could not be loaded."));
  }, [projectId]);
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><div className="text-xs font-bold uppercase tracking-wide text-brand-600">Trust & control</div><h1 className="mt-1 text-3xl font-bold text-slate-950">Approval Center</h1><p className="mt-2 max-w-3xl text-sm text-slate-600">Review AI recommendations and proposed changes before execution. Publishing, integrations, strategy changes, and high-risk actions always pause.</p></div>{tasks.length > 0 && <div className="flex gap-2 text-xs font-bold"><span className="rounded-full bg-amber-50 px-3 py-1.5 text-amber-700">{tasks.length} waiting</span><span className="rounded-full bg-red-50 px-3 py-1.5 text-red-700">{counts.high} high risk</span></div>}</div>
    ...
    {tasks.length > 0 && <Card className="flex flex-wrap items-center justify-between gap-3 p-4"><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={selected.length === tasks.length} onChange={(event) => setSelected(event.target.checked ? tasks.map((task) => task.id) : [])} /> Select all non-high-risk work</label><div className="flex gap-2"><button disabled={!counts.selected || busy === "bulk"} onClick={() => void bulkDecide("rejected")} className="rounded-lg border px-4 py-2 text-sm font-bold">Reject selected</button><button disabled={!counts.selected || busy === "bulk"} onClick={() => void bulkDecide("approved")} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-bold text-white">Approve selected</button></div></Card>}
    <div className="space-y-4">{tasks.map((task) => <Card key={task.id} className={`overflow-hidden border-l-4 ${task.highRisk ? "border-l-red-500" : "border-l-brand-500"}`}><div className="p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div className="flex min-w-0 gap-3"><input type="checkbox" checked={selected.includes(task.id)} disabled={task.highRisk} onChange={(event) => setSelected((current) => event.target.checked ? [...current, task.id] : current.filter((id) => id !== task.id))} className="mt-1" /><div><div className="flex flex-wrap gap-2"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold">{label(task.approvalType)}</span><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${task.highRisk ? "bg-red-50 text-red-700" : "bg-amber-50 text-amber-700"}`}>{label(task.effectiveRisk)} risk</span></div><h2 className="mt-3 text-lg font-bold text-slate-950">{task.title}</h2><p className="mt-1 text-xs text-slate-500">{task.project?.agencyClient?.name ? `${task.project.agencyClient.name} · ` : ""}{task.project?.name}</p></div></div>{task.relatedUrl && <Link to={task.relatedUrl} className="text-sm font-bold text-brand-700">Open source →</Link>}</div>
      ...
      <div className="mt-5 flex flex-wrap gap-2"><button disabled={busy === task.id} onClick={() => void decide(task, "approved")} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-bold text-white">✓ Approve</button><button disabled={busy === task.id} onClick={() => void decide(task, "edit_first")} className="rounded-lg border px-4 py-2 text-sm font-bold">✎ Edit First</button><button disabled={busy === task.id} onClick={() => void decide(task, "regenerate")} className="rounded-lg border px-4 py-2 text-sm font-bold">↻ Regenerate</button><button disabled={busy === task.id} onClick={() => void decide(task, "rejected")} className="rounded-lg border border-red-200 px-4 py-2 text-sm font-bold text-red-700">Reject</button></div>
    ...
    </div></Card>)}{projectId && !tasks.length && <Card className="p-10 text-center"><div className="text-3xl">✓</div><h2 className="mt-3 font-bold">Nothing is waiting for approval</h2><p className="mt-1 text-sm text-slate-500">New approval-gated work for this project will appear here automatically.</p></Card>}</div>
  </div>;
}
