...
type ApprovalDocument = { id: string; projectId: string; reportType: string; qaStatus: string; approvalStatus: string; currentVersion: number; updatedAt: string; project?: { name: string; agencyClient?: { name: string } | null } };
type ApprovalTask = {
...
export default function Approvals() {
  ...
  const load = async (id = projectId) => {
    ...
    const [result, historyResult] = await Promise.all([
      api.get<{ tasks: ApprovalTask[]; documents: ApprovalDocument[] }>(`/api/approvals${suffix}`),
      api.get<{ history: ApprovalHistoryItem[] }>(`/api/approvals/history${suffix}`),
    ]);
    setTasks(result.tasks); setDocuments(result.documents ?? []); setHistory(historyResult.history);
  ...
  return <div className="space-y-6">
    ...
    <Card className="p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-wide text-brand-700">Review queue</div><h2 className="mt-1 text-lg font-bold text-slate-950">Pending ({tasks.length + documents.length})</h2><p className="mt-1 text-xs text-slate-500">Protected actions for the selected project scope appear below.</p></div><details className="min-w-[260px] rounded-xl border border-slate-200 bg-slate-50 p-3"><summary className="cursor-pointer text-sm font-bold text-slate-800">History ({history.length})</summary><div className="mt-3 max-h-64 space-y-2 overflow-y-auto">{history.map((item) => <div key={item.id} className="rounded-lg border bg-white p-3 text-xs"><div className="font-bold text-slate-900">{item.task.title}</div><div className="mt-1 text-slate-500">{item.task.project?.name || "Project"} · {label(item.decision)} · {formatDisplayDate(item.createdAt)}</div>{item.notes && <p className="mt-1 leading-5 text-slate-600">{item.notes}</p>}</div>)}{!history.length && <p className="text-xs text-slate-500">No approval decisions have been recorded for this scope.</p>}</div></details></div></Card>
    {message && <div className="rounded-lg border border-brand-200 bg-brand-50 px-4 py-3 text-sm text-brand-800">{message}</div>}
    {documents.length > 0 && <Card className="p-5"><div className="text-xs font-black uppercase tracking-wide text-violet-700">Documents requiring review</div><div className="mt-3 space-y-2">{documents.map((document) => <div key={document.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-4"><div><div className="font-bold text-slate-950">{label(document.reportType)}</div><div className="mt-1 text-xs text-slate-500">{document.project?.agencyClient?.name ? document.project.agencyClient.name + " · " : ""}{document.project?.name} · QA {label(document.qaStatus)} · Version {document.currentVersion}</div></div><Link to={"/reports?projectId=" + encodeURIComponent(document.projectId) + "&reportId=" + encodeURIComponent(document.id)} className="rounded-lg bg-violet-600 px-4 py-2 text-sm font-bold text-white">Review document →</Link></div>)}</div></Card>}
    ...
    <div className="space-y-4">{tasks.map((task) => <Card key={task.id} className={`overflow-hidden border-l-4 ${task.highRisk ? "border-l-red-500" : "border-l-brand-500"}`}><div className="p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div className="flex min-w-0 gap-3">{!task.confirmationOnly && <input type="checkbox" checked={selected.includes(task.id)} disabled={task.highRisk} onChange={(event) => setSelected((current) => event.target.checked ? [...current, task.id] : current.filter((id) => id !== task.id))} className="mt-1" />}<div><div className="flex flex-wrap gap-2"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold">{task.confirmationOnly ? "Owner confirmation" : label(task.approvalType)}</span><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${task.highRisk ? "bg-red-50 text-red-700" : "bg-amber-50 text-amber-700"}`}>{label(task.effectiveRisk)} risk</span></div><h2 className="mt-3 text-lg font-bold text-slate-950">{task.title}</h2><p className="mt-1 text-xs text-slate-500">{task.project?.agencyClient?.name ? `${task.project.agencyClient.name} · ` : ""}{task.project?.name}</p></div></div>{task.relatedUrl && <Link to={task.relatedUrl} className="text-sm font-bold text-brand-700">Open source →</Link>}</div>
      ...
      {task.approvalHistory.length > 0 && <details className="mt-5 border-t pt-4"><summary className="cursor-pointer text-sm font-bold">Approval history ({task.approvalHistory.length})</summary><div className="mt-3 space-y-2">{task.approvalHistory.map((event) => <div key={event.id} className="rounded-lg bg-slate-50 p-3 text-xs"><b>{label(event.decision)}</b> by {event.actorMembership?.user?.name || event.actorMembership?.user?.email || "Workspace user"} · {formatDisplayDate(event.createdAt)}{event.notes && <p className="mt-1 text-slate-600">{event.notes}</p>}</div>)}</div></details>}
