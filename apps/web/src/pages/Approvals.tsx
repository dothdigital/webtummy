...
export default function Approvals() {
  ...
  return <div className="space-y-6">
    <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-950 px-6 py-7 text-white shadow-xl sm:px-8"><div className="pointer-events-none absolute inset-0 opacity-30 [background-image:linear-gradient(rgba(45,212,191,0.12)_1px,transparent_1px),linear-gradient(90deg,rgba(45,212,191,0.12)_1px,transparent_1px)] [background-size:72px_72px]" /><div className="relative flex flex-wrap items-end justify-between gap-4"><div><div className="text-xs font-bold uppercase tracking-wide text-emerald-300">Trust &amp; control</div><h1 className="mt-1 text-3xl font-bold text-white">Approval Center</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">Review AI-prepared changes before execution. Personal owners confirm their own protected actions; Business and Agency teams can route work to an approver.</p></div>{tasks.length > 0 && <div className="flex gap-2 text-xs font-bold"><span className="rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1.5 text-amber-200">{tasks.length} waiting</span><span className="rounded-full border border-red-300/20 bg-red-300/10 px-3 py-1.5 text-red-200">{counts.high} high risk</span></div>}</div></section>
