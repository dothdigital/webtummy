...
export default function Approvals() {
  ...
  return <div className="space-y-6">
    ...
    {tasks.some((task) => !task.highRisk && !task.confirmationOnly) && <Card className="flex flex-wrap items-center justify-between gap-3 p-4"><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={selected.length > 0 && selected.length === tasks.filter((task) => !task.highRisk && !task.confirmationOnly).length} onChange={(event) => setSelected(event.target.checked ? tasks.filter((task) => !task.highRisk && !task.confirmationOnly).map((task) => task.id) : [])} /> Select all standard team approvals</label><div className="flex gap-2"><button disabled={!counts.selected || busy === "bulk"} onClick={() => void bulkDecide("rejected")} className="rounded-lg border px-4 py-2 text-sm font-bold">Reject selected</button><button disabled={!counts.selected || busy === "bulk"} onClick={() => void bulkDecide("approved")} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-bold text-white">Approve selected</button></div></Card>}
