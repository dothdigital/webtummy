...
const clientProjectTypes = [
  ...
  { value: "ecommerce_store", label: "Ecommerce Store", description: "Online store with products, collections, buyer journeys, and sales-focused growth.", projectType: "ecommerce" },
  { value: "marketplace_seller", label: "Marketplace Seller", description: "Amazon, Etsy, eBay, Poshmark, or another marketplace-led product business.", projectType: "ecommerce" },
  { value: "digital_product", label: "Digital Product Business", description: "Courses, downloads, memberships, templates, or other digital products.", projectType: "existing_website" },
  { value: "saas_software", label: "SaaS or Software", description: "Software, platform, app, or technology product with acquisition and conversion goals.", projectType: "existing_website" },
  { value: "content_affiliate", label: "Content or Affiliate Business", description: "Publisher, niche site, affiliate content, topical authority, and monetized content growth.", projectType: "existing_website" },
...
export default function GuidedProjectNew() {
    ...
    void api.get<{ project: GuidedProject }>(`/api/projects-v2/${loadProjectId}`)
      .then(({ project }) => {
        ...
        patch({
          ...
          name: project.name,
          clientProjectType: project.projectType === "ecommerce" ? "ecommerce_store" : project.projectType === "local_seo" ? "local_business" : "service_business",
  ...
  if (!editProjectId && creationMode === "choose") return <div className="space-y-6">
    ...
    <div className="grid gap-5 md:grid-cols-3">
      {[
        { key:"EXISTING_BUSINESS" as const, icon:"↗", badge:"Already operating", title:"I have an existing business or online store", detail:"Describe your business or provide an optional website, online store, marketplace listing or business profile. SEnuke AI will understand what already exists and recommend the best practical direction.", points:["Known facts are reused","Location is asked only when relevant"], tone:"border-cyan-300 bg-gradient-to-br from-cyan-50 via-white to-blue-50 hover:border-cyan-600 hover:ring-4 hover:ring-cyan-100 focus-visible:border-cyan-700 focus-visible:ring-4 focus-visible:ring-cyan-200", iconTone:"bg-cyan-700 text-white" },
        { key:"IDEA_TO_EXPLORE" as const, icon:"⌕", badge:"Explore first", title:"I have a business idea to explore", detail:"Refine an incomplete business or ecommerce idea, audience, revenue model and first validation step before deciding whether to create a project.", points:["Autosaved Pre-Project Brief","No Project until Use This Idea"], tone:"border-violet-300 bg-gradient-to-br from-violet-50 via-white to-fuchsia-50 hover:border-violet-600 hover:ring-4 hover:ring-violet-100 focus-visible:border-violet-700 focus-visible:ring-4 focus-visible:ring-violet-200", iconTone:"bg-violet-700 text-white" },
      ...
      ].map((option)=><button key={option.key} type="button" disabled={startingDiscovery || form.name.trim().length < 2 || (isAgency && !form.agencyClientId)} onClick={()=>option.key === "EXISTING_BUSINESS" ? setCreationMode("classic") : void startDiscovery(option.key)} className={`group flex h-full min-h-[390px] flex-col rounded-[24px] border-2 p-7 text-left shadow-sm outline-none transition hover:-translate-y-1 hover:shadow-xl focus-visible:-translate-y-1 focus-visible:shadow-xl disabled:cursor-not-allowed disabled:opacity-50 sm:p-8 ${option.tone}`}><div className="flex items-start justify-between gap-4"><span className={`grid h-14 w-14 place-items-center rounded-2xl text-2xl shadow-lg ${option.iconTone}`}>{option.icon}</span><span className="rounded-full bg-white/90 px-3 py-1.5 text-[10px] font-black uppercase tracking-wide text-slate-700 shadow-sm">{option.badge}</span></div><h2 className="mt-6 text-2xl font-black text-slate-950">{option.title}</h2><p className="mt-2 text-sm font-medium leading-7 text-slate-800">{option.detail}</p><div className="mt-5 space-y-2">{option.points.map((item)=><div key={item} className="flex items-start gap-3 text-xs font-bold leading-5 text-slate-800"><span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-white text-[10px] text-emerald-700">✓</span>{item}</div>)}</div><div className="mt-auto pt-7"><div className="flex items-center justify-between border-t border-slate-300/80 pt-5"><span className="text-xs font-bold text-slate-700">Business Discovery</span><span className="rounded-lg bg-slate-950 px-3 py-2 text-sm font-black text-white shadow-sm transition group-hover:translate-x-1 group-hover:bg-brand-700">{startingDiscovery ? "Starting…" : "Continue →"}</span></div></div></button>)}
    </div>
