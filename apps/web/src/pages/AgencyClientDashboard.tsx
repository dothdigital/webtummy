...
type ClientDashboard = {
  ...
  projects: { id: string; name: string; projectType: string; status: string; currentStep: string; primaryGoal: string | null; websiteUrl: string | null }[];
...
export default function AgencyClientDashboard() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><Link to="/agency?tab=clients" className="text-xs font-bold text-brand-700">← Agency Workspace</Link><h1 className="mt-2 text-3xl font-bold">{data.client.name}</h1><p className="mt-2 text-sm text-slate-600">{data.client.contactEmail || "No contact email"} · {arrayText(data.client.targetMarkets) || "No target markets"}</p></div>{data.permissions.canManage && <Link to={`/projects/new?agencyClientId=${data.client.id}`} className="inline-flex h-10 items-center rounded-lg bg-brand-600 px-4 text-sm font-bold text-white">Create project</Link>}</div>
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
      <div className="space-y-6">
        <Card className="p-5"><h2 className="font-bold">Projects</h2><div className="mt-4 grid gap-3 md:grid-cols-2">{data.projects.map((project) => <Link key={project.id} to={data.permissions.clientViewer ? "#" : `/guided-projects/${project.id}`} className="rounded-lg border p-4"><div className="flex justify-between gap-3"><b>{project.name}</b><span className="text-xs font-bold text-brand-700">{label(project.currentStep)}</span></div><p className="mt-2 text-sm text-slate-500">{project.primaryGoal || label(project.projectType)}</p></Link>)}</div></Card>
        <Card className="p-5"><h2 className="font-bold">Approval requests</h2><div className="mt-4 space-y-3">{pending.length ? pending.map((task) => <div key={task.id} className="rounded-lg border p-4"><div className="flex flex-wrap justify-between gap-2"><div><b>{task.title}</b><p className="mt-1 text-sm text-slate-600">{task.clientVisibleNotes || "Client approval requested."}</p></div><span className="h-fit rounded-full bg-amber-50 px-2.5 py-1 text-xs font-bold text-amber-700">{label(task.approvalRisk)} risk</span></div>{data.permissions.clientViewer && task.clientApprovalRequired && <div className="mt-4 flex gap-2"><button disabled={busy === task.id} onClick={() => action(task.id, () => api.post(`/api/agency/tasks/${task.id}/decision`, { decision: "approved", snapshotJson: {} }), "Approval recorded.")} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white">Approve</button><button disabled={busy === task.id} onClick={() => action(task.id, () => api.post(`/api/agency/tasks/${task.id}/decision`, { decision: "changes_requested", notes: "Client requested changes.", snapshotJson: {} }), "Change request recorded.")} className="rounded-lg border px-3 py-2 text-xs font-bold">Request changes</button></div>}</div>) : <p className="text-sm text-slate-500">No approval requests are waiting.</p>}</div></Card>
