...
function KeywordScreen({ data }: { data: ModuleData }) {
  ...
  const nextStep = needsSiteAnalysis
    ...
    : { title: "Continue to Strategy", detail: "Your approved keyword direction is ready and will now guide Strategy, content priorities, and the Execution Plan.", to: `/strategy?projectId=${project?.id ?? ""}`, label: "Generate Strategy" };
  return (
    <>
      {runs.length > 0 && <>
      ...
      {approvedCount > 0 && runs.length > 0 && <Card className="overflow-hidden border-emerald-200 bg-gradient-to-r from-emerald-50 via-white to-brand-50"><div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-emerald-600 font-bold text-white">✓</div><div><div className="text-xs font-bold uppercase tracking-wide text-emerald-700">Keyword analysis available</div><h2 className="mt-1 text-lg font-bold text-charcoal-950">{nextStep.title}</h2><p className="mt-1 text-sm leading-6 text-charcoal-600">{nextStep.detail}</p><p className="mt-1 text-xs font-semibold text-charcoal-500">{approvedCount} group{approvedCount === 1 ? "" : "s"} approved and {runs.length} analysis run{runs.length === 1 ? "" : "s"} available.</p></div></div><Link to={nextStep.to} className="inline-flex shrink-0 items-center justify-center rounded-lg bg-brand-600 px-5 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-brand-700">{nextStep.label} →</Link></div></Card>}
      <Card className={`${runs.length === 0 && approvedCount > 0 ? "border-brand-200 bg-gradient-to-r from-brand-50 via-white to-emerald-50" : ""} p-5`}><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-xs font-bold uppercase tracking-wide text-brand-600">Keyword Intelligence Groups</div><h2 className="mt-1 text-lg font-bold text-charcoal-950">{runs.length === 0 && approvedCount > 0 ? "Approved direction ready for analysis" : "Approve and manage keyword direction"}</h2><p className="mt-1 text-sm leading-6 text-charcoal-600">{runs.length === 0 && approvedCount > 0 ? "Use the approved groups to collect demand, competition, CPC, intent, rankings, and page-target data." : "Review, approve, edit, or add keywords before continuing."}</p><p className="mt-1 text-xs font-semibold text-charcoal-500">{groups.length} groups · {approvedCount} approved · {totalRecommendations} recommendations · {isExistingWebsiteFlow(project, website) ? "existing website context" : "crawl not required"}</p></div>{runs.length === 0 && approvedCount > 0 && <Link to={keywordAnalysisTo} className="inline-flex shrink-0 items-center justify-center rounded-lg bg-brand-600 px-5 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-brand-700">Start Keyword Analysis →</Link>}</div></Card>
