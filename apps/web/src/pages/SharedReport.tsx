...
export default function SharedReport({ token }: { token: string }) {
  ...
  return <main className="min-h-screen bg-slate-100 py-8"><article className="mx-auto max-w-5xl overflow-hidden rounded-2xl bg-white shadow-xl"><header className="bg-slate-950 px-8 py-9 text-white"><div className="text-xs font-black uppercase tracking-[0.16em] text-cyan-300">Secure client-safe report</div><h1 className="mt-3 text-3xl font-black">{data.report.title}</h1><p className="mt-2 text-sm text-slate-300">{data.report.projectName} · Version {data.report.version}{data.report.periodStart ? ` · ${new Date(data.report.periodStart).toLocaleDateString()}–${new Date(data.report.periodEnd || data.report.periodStart).toLocaleDateString()}` : ""}</p></header><div className="space-y-7 p-8">{Boolean(narrative.executiveNarrative) && <section><h2 className="text-xl font-black text-slate-950">Executive summary</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{String(narrative.executiveNarrative)}</p></section>}{sections.map((section) => { const items = Array.isArray(section.items) ? section.items : []; const metrics = Array.isArray(section.metrics) ? section.metrics.map(record) : []; return <section key={String(section.key)} className="border-t border-slate-200 pt-6"><h2 className="text-xl font-black text-slate-950">{String(section.title)}</h2>{Boolean(section.summary) && <p className="mt-2 text-sm leading-6 text-slate-700">{String(section.summary)}</p>}{metrics.length > 0 && <div className="mt-4 grid gap-3 sm:grid-cols-3">{metrics.map((metric, index) => <div key={index} className="rounded-xl border bg-slate-50 p-4"><div className="text-xl font-black">{String(metric.value ?? "Data pending")}</div><div className="mt-1 text-xs font-bold uppercase text-slate-500">{String(metric.label)}</div></div>)}</div>}{items.length > 0 ? <ul className="mt-3 list-disc space-y-1.5 pl-5 text-sm leading-6 text-slate-700">{items.map((item, index) => <li key={index}>{typeof item === "object" ? String(record(item).title || record(item).recommendation || record(item).keyword || "Recorded evidence") : String(item)}</li>)}</ul> : Boolean(section.emptyMessage) && <p className="mt-3 rounded-lg border border-dashed bg-slate-50 p-3 text-xs text-slate-500">{String(section.emptyMessage)}</p>}</section>; })}<div className="flex flex-wrap items-center justify-between gap-3 border-t pt-6"><p className="text-xs text-slate-500">This link expires {formatDisplayDate(data.report.expiresAt, { includeTime: true })}.</p><a href={`/api/public/reports/${encodeURIComponent(token)}/download`} className="rounded-lg bg-cyan-600 px-4 py-2 text-sm font-bold text-white">Download PDF</a></div></div></article></main>;
}
