  ...
  return <div className="space-y-6">
    <div className="rounded-2xl border border-slate-200 bg-white px-5 py-5 shadow-sm lg:px-7 lg:py-6">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div className="flex shrink-0 flex-nowrap gap-2 overflow-x-auto pb-1"><Link to="/" className="inline-flex h-10 shrink-0 items-center rounded-lg border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 hover:border-slate-300 hover:bg-slate-50">My Dashboard</Link>{canAdmin && isAgency && <button onClick={() => { openTab("clients"); setShowClientForm(true); }} className="h-10 shrink-0 rounded-lg border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 hover:bg-slate-50">New client</button>}{isAgency && !data.clients.some((client) => client.status === "active") ? <button type="button" onClick={() => { openTab("clients"); setShowClientForm(true); }} className="inline-flex h-10 shrink-0 items-center rounded-lg bg-brand-600 px-4 text-sm font-bold text-white shadow-sm hover:bg-brand-700">Add client before project</button> : <Link to="/projects/new" className="inline-flex h-10 shrink-0 items-center rounded-lg bg-brand-600 px-4 text-sm font-bold text-white shadow-sm hover:bg-brand-700">New project</Link>}</div>
      </div>
    ...
    {isAgency && tab === "clients" && <div className="space-y-6">
      ...
      {canAdmin && showClientForm && <Card className="p-6 lg:p-8"><div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-bold">Create client</h2><p className="mt-1 text-sm text-slate-500">This shared client profile is reused across every project.</p></div><button type="button" onClick={() => setShowClientForm(false)} className="rounded-lg border px-4 py-2 text-sm font-bold text-slate-600">Cancel</button></div><form onSubmit={(event) => void createClient(event)} className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        ...
        <ClientField label="Country *" value={clientCountry} onChange={setClientCountry} required /><ClientField label="State / Province *" value={clientRegion} onChange={setClientRegion} required /><ClientField label="City *" value={clientCity} onChange={setClientCity} required />
