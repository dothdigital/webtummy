...
export default function AgencyWorkspace() {
  ...
  async function setProjectMember(project: Project, membershipId: string, assigned: boolean) {
    const currentMembershipIds = project.memberAssignments.map((item) => item.membershipId);
    const membershipIds = assigned
      ? [...new Set([...currentMembershipIds, membershipId])]
      : currentMembershipIds.filter((id) => id !== membershipId);
    await action(
      `project-${project.id}-${membershipId}`,
      () => api.put(`/api/agency/projects/${project.id}/assignments`, { membershipIds, teamIds: project.teamAssignments.map((item) => item.teamId) }),
  ...
  return <div className="space-y-6">
    ...
    {tab === "teams" && <div className="space-y-6">
      ...
      {isAgency && canAdmin && <Card className="p-5"><h2 className="font-bold">Project assignments</h2><p className="mt-1 text-xs text-slate-500">Select the workspace users who should access and work on each project. Changes save immediately.</p><div className="mt-4 space-y-3">{portfolioProjects.map((project) => <div key={project.id} className="rounded-lg border p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><div className="text-sm font-bold text-slate-950">{project.name}</div><div className="mt-1 text-xs text-slate-500">{project.client?.name || "Agency client"}</div></div><Badge tone="blue">{project.memberAssignments.length} assigned</Badge></div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{data.members.filter((member) => member.status === "active" && !(normalizedRoles(member.roles).length === 1 && normalizedRoles(member.roles)[0] === "client_viewer")).map((member) => { const checked = project.memberAssignments.some((item) => item.membershipId === member.id); const actionKey = `project-${project.id}-${member.id}`; return <label key={member.id} className="flex cursor-pointer items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm hover:bg-slate-50"><input type="checkbox" checked={checked} disabled={busy === actionKey} onChange={(event) => void setProjectMember(project, member.id, event.target.checked)} className="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500" /><span className="min-w-0 truncate font-semibold">{member.user.name || member.user.email}</span></label>; })}</div></div>)}{!portfolioProjects.length && <div className="rounded-lg border border-dashed p-6 text-center text-sm text-slate-500">No active projects are available for assignment.</div>}</div></Card>}
