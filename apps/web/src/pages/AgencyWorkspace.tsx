...
export default function AgencyWorkspace() {
  ...
  return <div className="space-y-6">
    ...
    {tab === "teams" && <div className="space-y-6">
      <div className="grid gap-6 xl:grid-cols-[1fr_340px]"><Card className="p-5"><h2 className="font-bold">Teams</h2><div className="mt-4 grid gap-4 md:grid-cols-2">{data.teams.map((team) => <div key={team.id} className="rounded-lg border p-4"><div className="flex justify-between"><div><h3 className="font-bold">{team.name}</h3><p className="mt-1 text-xs text-slate-500">{team.description || "No description"}</p></div><Badge>{team.members.length}</Badge></div><div className="mt-4 flex flex-wrap gap-2">{team.members.map((item) => <Badge key={item.membership.id} tone="blue">{item.membership.user.name || item.membership.user.email}</Badge>)}</div></div>)}</div></Card>
