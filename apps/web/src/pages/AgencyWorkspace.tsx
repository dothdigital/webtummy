...
import { businessFirstUseSupportingText, customerPlanLabel, guidedSetupSteps, projectAllowanceLabel, workspaceDisplayName, workspaceProjectActivityCopy, workspaceProjectAssignmentLabel, workspaceStartingPathEmphasized, workspaceStartingPaths, type GuidedSetupStep } from "../workspace-dashboard.js";
...
export function GuidedSetup({ steps, preview = false, agency = false }: { steps: GuidedSetupStep[]; preview?: boolean; agency?: boolean }) {
  ...
  return <Card className="overflow-hidden border-brand-200 p-0">
    <div className="bg-gradient-to-br from-brand-50 via-white to-violet-50 p-5 sm:p-7">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="max-w-3xl"><div className="text-xs font-black uppercase tracking-[0.16em] text-brand-700">{ready ? "Your workspace is ready." : "Guided setup"}</div><h2 className="mt-2 text-2xl font-black text-slate-950">{ready ? "Open your Next Best Action" : agency ? "Let’s confirm the client profile and determine what should happen next." : "Let’s understand your business and determine what should happen next."}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{agency ? "Complete the client setup once. SEnuke AI will use the client's verified information and available evidence to build a customized Strategy and recommend the first Next Best Action." : "Complete the guided setup once. SEnuke AI will use your verified business information and available evidence to build a customized Strategy and recommend the first Next Best Action."}</p></div>
...
export default function AgencyWorkspace() {
  ...
  const portfolioCompletion = projectProgressPercentages.length
    ...
    : 0;
  // Legacy records may have been closed while execution actions were still
  // open. Do not report those projects as complete on the dashboard.
  const completedProjects = portfolioProjects.filter((project) => project.status === "completed" && project.actionProgress.remaining === 0).length;
  ...
  const setupSteps = guidedSetupSteps({ workspaceType: data?.workspace.workspaceType ?? "personal", activeClientCount: data?.clients.filter((client) => client.status === "active").length ?? 0, project: firstProject ?? null, approvalMode: data?.approvalMode, governanceConfirmed: data?.governanceConfirmed });
  ...
  function focusInviteForm() {
    inviteFormRef.current?.scrollIntoView({ block: "center" });
    inviteFormRef.current?.querySelector<HTMLInputElement>("input")?.focus({ preventScroll: true });
  }
  useEffect(() => {
    if (showInviteForm && tab === "teams") focusInviteForm();
  }, [showInviteForm, tab]);

  function openInviteForm(teamId = "") {
    setInviteTeamId(teamId);
    setShowInviteForm(true);
    if (showInviteForm) focusInviteForm();
  }
  ...
  async function inviteMember(event: React.FormEvent) {
    event.preventDefault();
    const sent = await action("invite", () => api.post("/api/workspace/invitations", {
  ...
  return <div className="space-y-6">
    <div className="rounded-2xl border border-slate-200 bg-white px-5 py-5 shadow-sm lg:px-7 lg:py-6">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0">
          {isAgency && <div className="flex flex-wrap items-center gap-2 text-xs font-bold text-slate-400"><Link to="/workspace" className="text-brand-700 hover:text-brand-800">Agency Portfolio</Link></div>}
    ...
    {tab === "dashboard" && <>
      <GuidedSetup steps={setupSteps} agency={isAgency} />
    ...
    {isAgency && tab === "clients" && <div className="space-y-6">
      ...
      {data.clients.length > 0 && !showClientForm && <Card className="overflow-hidden p-0"><div className="flex flex-wrap items-center justify-between gap-3 border-b px-5 py-4"><div><h2 className="font-bold">Clients</h2><p className="text-xs text-slate-500">Client information is reused only by projects belonging to that client.</p></div><div className="flex gap-2">{(["active", "archived"] as const).map((status) => <button key={status} type="button" onClick={() => setClientFilter(status)} className={`rounded-full border px-4 py-2 text-xs font-bold ${clientFilter === status ? "border-brand-300 bg-brand-50 text-brand-800" : "border-slate-200 bg-white text-slate-500"}`}>{status === "active" ? "Active" : "Archived"} ({data.clients.filter((client) => client.status === status).length})</button>)}</div></div><div className="divide-y">{filteredClients.map((client) => <div key={client.id} className="p-5">
      ...
      {canManageClients && showClientForm && <Card className="p-6 lg:p-8"><div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-bold">Create client</h2><p className="mt-1 text-sm text-slate-500">Client information is reused only by projects belonging to that client.</p></div><button type="button" onClick={() => setShowClientForm(false)} className="rounded-lg border px-4 py-2 text-sm font-bold text-slate-600">Cancel</button></div><div className="mt-6 grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_340px]"><form onSubmit={(event) => void createClient(event)} className="grid gap-5 md:grid-cols-2">
