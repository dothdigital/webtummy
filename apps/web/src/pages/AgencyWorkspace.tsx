...
export default function AgencyWorkspace() {
  ...
  return <div className="space-y-6">
    ...
    {isAgency && tab === "clients" && <div className="space-y-6">
      {!showClientForm && <div className={`flex ${data.clients.length ? "justify-end" : "min-h-[45vh] items-center justify-center"}`}>{canAdmin && <button type="button" onClick={() => setShowClientForm(true)} className="inline-flex h-11 items-center rounded-xl bg-brand-600 px-6 text-sm font-bold text-white shadow-sm hover:bg-brand-700">+ Create Client</button>}</div>}
      ...
      {canAdmin && showClientForm && <Card className="p-6 lg:p-8"><div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-bold">Create client</h2><p className="mt-1 text-sm text-slate-500">This shared client profile is reused across every project.</p></div><button type="button" onClick={() => setShowClientForm(false)} className="rounded-lg border px-4 py-2 text-sm font-bold text-slate-600">Cancel</button></div><form onSubmit={(event) => void createClient(event)} className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
    ...
    {tab === "teams" && <div className="space-y-6">
      ...
      {canAdmin && showInviteForm && <Card className="p-6 lg:p-8"><div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-bold">Invite user</h2><p className="mt-1 text-sm text-slate-500">Choose their workspace role and optional team or client assignment. They will receive an email to create their own login.</p></div><button type="button" onClick={() => setShowInviteForm(false)} className="rounded-lg border px-4 py-2 text-sm font-bold text-slate-600">Cancel</button></div><form onSubmit={(event) => void inviteMember(event)} className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3"><label className="text-xs font-bold">Name<input value={inviteName} onChange={(event) => setInviteName(event.target.value)} className="mt-1 h-10 w-full rounded-lg border px-3 text-sm font-normal" /></label><label className="text-xs font-bold">Email *<input required type="email" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} className="mt-1 h-10 w-full rounded-lg border px-3 text-sm font-normal" /></label><label className="text-xs font-bold">Role<select value={inviteRole} onChange={(event) => setInviteRole(event.target.value as Role)} className="mt-1 h-10 w-full rounded-lg border bg-white px-3 text-sm font-normal">{allowedRoles.filter((role) => role !== "owner").map((role) => <option key={role} value={role}>{roleLabels[role]}</option>)}</select></label><label className="text-xs font-bold">Team<select value={inviteTeamId} onChange={(event) => setInviteTeamId(event.target.value)} className="mt-1 h-10 w-full rounded-lg border bg-white px-3 text-sm font-normal"><option value="">No team</option>{data.teams.map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select></label><label className="text-xs font-bold">Client {inviteRole === "client_viewer" ? "*" : ""}<select required={inviteRole === "client_viewer"} value={inviteClientId} onChange={(event) => setInviteClientId(event.target.value)} className="mt-1 h-10 w-full rounded-lg border bg-white px-3 text-sm font-normal"><option value="">No client</option>{data.clients.filter((client) => client.status === "active").map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select></label><div className="flex items-end justify-end gap-3 md:col-span-2 xl:col-span-1"><button type="button" onClick={() => setShowInviteForm(false)} className="h-10 rounded-lg border px-4 text-sm font-bold text-slate-600">Cancel</button><button disabled={busy === "invite"} className="h-10 rounded-lg bg-brand-600 px-5 text-sm font-bold text-white disabled:bg-slate-300">{busy === "invite" ? "Sending…" : "Send Invitation"}</button></div></form></Card>}
