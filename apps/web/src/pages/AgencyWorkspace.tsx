...
import { configurableWorkspaceRoles, defaultWorkspacePermission, workspacePermissionCatalog, type ConfigurableWorkspaceRole } from "@webtummy/core/workspace-permissions";
  ...
  return <div className="space-y-6">
    <div className="rounded-2xl border border-slate-200 bg-white px-5 py-5 shadow-sm lg:px-7 lg:py-6">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div className="flex shrink-0 flex-nowrap gap-2 overflow-x-auto pb-1"><Link to="/" className="inline-flex h-10 shrink-0 items-center rounded-lg border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 hover:border-slate-300 hover:bg-slate-50">My Dashboard</Link>{isAgency && !data.clients.some((client) => client.status === "active") ? canManageClients && <button type="button" onClick={() => { openTab("clients"); setShowClientForm(true); }} className="inline-flex h-10 shrink-0 items-center rounded-lg bg-brand-600 px-4 text-sm font-bold text-white shadow-sm hover:bg-brand-700">Create Client</button> : <>{canManageClients && isAgency && <button onClick={() => { openTab("clients"); setShowClientForm(true); }} className="h-10 shrink-0 rounded-lg border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 hover:bg-slate-50">New Client</button>}{canCreateProjects && <Link to="/projects/new" className="inline-flex h-10 shrink-0 items-center rounded-lg bg-brand-600 px-4 text-sm font-bold text-white shadow-sm hover:bg-brand-700">New Project</Link>}</>}</div>
      </div>
