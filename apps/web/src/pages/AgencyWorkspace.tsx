...
import { customerPlanLabel, personalStartingPaths, projectAllowanceLabel } from "../workspace-dashboard.js";
...
export default function AgencyWorkspace() {
  ...
  const rawWorkspaceName = data?.workspace.name.trim() ?? "";
  const workspaceName = rawWorkspaceName && rawWorkspaceName.toLowerCase() !== data?.workspace.workspaceType.toLowerCase() ? rawWorkspaceName : "My Workspace";
  ...
  return <div className="space-y-6">
    ...
    {tab === "dashboard" && <>
      <AttentionQueue actions={dashboardActions} />
      {!portfolioProjects.length && !isAgency && <Card className="overflow-hidden border-brand-100 p-0">
        <div className="bg-gradient-to-br from-brand-50 via-white to-emerald-50 px-5 py-6 sm:px-7 sm:py-8">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Tell us where you are starting. SEnuke AI will guide you from there.</p>
          {canCreateProjects ? <div className="mt-6 grid gap-3 md:grid-cols-3">{personalStartingPaths.map((path, index) => <Link key={path.key} to={path.href} aria-label={`${path.title}. ${path.detail}`} className="group flex min-h-40 flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-md focus:outline-none focus:ring-4 focus:ring-brand-100"><span className="grid h-9 w-9 place-items-center rounded-xl bg-slate-950 text-sm font-black text-white">{index + 1}</span><h3 className="mt-4 font-black text-slate-950">{path.title}</h3><p className="mt-2 flex-1 text-xs leading-5 text-slate-600">{path.detail}</p><span className="mt-4 text-sm font-black text-brand-700">Continue →</span></Link>)}</div> : <p className="mt-5 rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-600">You have read-only access. Ask a workspace administrator to start the first project.</p>}
        </div>
      ...
      {portfolioProjects.length > 0 && <Card id="client-project-actions" className="p-5">
        <h2 className="font-bold">{isAgency ? "Client and project actions" : "Project activity"}</h2>
        <p className="mt-1 text-sm text-slate-500">Live totals from your project’s Execution Plan. Select a project to review its status and continue the next action.</p>
