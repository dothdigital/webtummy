...
export function guidedSetupSteps(input: { workspaceType: string; activeClientCount: number; project: GuidedSetupProject | null; approvalMode?: string | null }): GuidedSetupStep[] {
  ...
  return [
    ...
    { key: "evidence", title: "Connect relevant accounts", detail: "Review website, analytics, search, local and publishing evidence. Optional connections may be deferred.", state: evidenceComplete ? "complete" : evidenceBlocked ? "blocked" : profileComplete ? "in_progress" : "not_started", href: evidenceHref },
