...
function RegisteredComponent({ componentId, props, mediaAssets, chrome }: { componentId: string; props: PuckProps; mediaAssets: VisualMediaAsset[]; chrome?: WebsiteChrome }) {
  ...
  if (componentId === "content.link_section") return <SectionShell tone="soft" className={`senuke-internal-link-section ${alignmentClass}`}><div><h2 style={{ fontFamily: "var(--senuke-heading-font)", fontSize: "clamp(30px,4vw,46px)" }}>{heading || "Related information"}</h2>{textParagraphs(props.introduction).map((paragraph, index) => <p key={`intro-${index}`} style={{ maxWidth: 900, color: "var(--senuke-muted)", fontSize: 17, lineHeight: 1.8 }}>{paragraph}</p>)}<div style={{ display: "grid", gridTemplateColumns: props.variant === "cards" ? "repeat(auto-fit,minmax(220px,1fr))" : "1fr", gap: props.variant === "cards" ? 12 : 8, margin: "24px 0" }}>{list(props.links).map((item, index) => <WebsiteActionLink key={index} href={text(item.url,"#")} chrome={chrome} style={props.variant === "cards" ? { border: "1px solid #dbeafe", borderRadius: "var(--senuke-radius)", background: "var(--senuke-surface)", padding: "15px 17px", color: "var(--senuke-primary)", fontWeight: 850, textDecoration: "none" } : { color: "var(--senuke-primary)", fontWeight: 850, textUnderlineOffset: 3 }}>{text(item.label || item.title, `Related page ${index + 1}`)}</WebsiteActionLink>)}</div>{textParagraphs(props.closingText).map((paragraph, index) => <p key={`closing-${index}`} style={{ maxWidth: 900, color: "var(--senuke-muted)", fontSize: 17, lineHeight: 1.8 }}>{paragraph}</p>)}</div></SectionShell>;
...
export function createSenukePuckConfig(theme: Theme = {}, mediaAssets: VisualMediaAsset[] = [], chrome?: WebsiteChrome): Config<any> {
  ...
  return {
    ...
    categories: {
      ...
      content: { title: "Content", components: ["content.rich_text", "content.link_section", "content.process", "content.faq"] },
