...
function registryField(fieldName: string, definition: { type: string; maxItems?: number }, mediaAssets: VisualMediaAsset[]) {
...
function RegisteredComponent({ componentId, props, mediaAssets, chrome }: { componentId: string; props: PuckProps; mediaAssets: VisualMediaAsset[]; chrome?: WebsiteChrome }) {
  ...
  if (componentId === "hero.local_service") return <SectionShell tone="soft" className="senuke-hero-section"><div style={{ display: "grid", gridTemplateColumns: imageAsset?.sourceUrl ? "minmax(0,1.15fr) minmax(280px,.85fr)" : "1fr", alignItems: "center", gap: "clamp(28px,6vw,70px)" }}><div style={{ maxWidth: 980 }}><div style={{ color: "var(--senuke-secondary)", fontSize: 12, fontWeight: 900, letterSpacing: ".14em", textTransform: "uppercase" }}>{text(props.eyebrow, "Service")}</div><h1 style={{ margin: "18px 0", fontFamily: "var(--senuke-heading-font)", fontSize: "clamp(40px, 6vw, 72px)", lineHeight: 1.04 }}>{text(props.headline, "Clear service headline")}</h1><p style={{ maxWidth: 780, color: "var(--senuke-muted)", fontSize: 19, lineHeight: 1.75 }}>{text(props.summary, "Explain the service, audience, and desired result.")}</p><WebsiteActionLink href={text(props.primaryCtaUrl, "/contact/")} chrome={chrome} style={{ display: "inline-block", marginTop: 18, borderRadius: "var(--senuke-radius)", background: "var(--senuke-primary)", color: "#fff", padding: "14px 22px", fontWeight: 900, textDecoration: "none", boxShadow: "0 14px 35px color-mix(in srgb,var(--senuke-primary) 35%,transparent)" }}>{text(props.primaryCtaLabel, "Get started")}</WebsiteActionLink></div>{imageAsset?.sourceUrl&&<img src={imageAsset.sourceUrl} alt={imageAsset.altText||text(props.headline)} style={{ width: "100%", aspectRatio: "3 / 2", maxHeight: 560, objectFit: "cover", borderRadius: "calc(var(--senuke-radius) * 1.4)", boxShadow: "0 30px 80px rgba(15,23,42,.2)" }}/>}</div></SectionShell>;
  if (componentId === "content.rich_text") return <SectionShell className={`senuke-rich-section senuke-rich-${text(props.variant,"standard")}`}><div><h2 style={{ fontFamily: "var(--senuke-heading-font)", fontSize: "clamp(30px,4vw,46px)" }}>{heading || "Content section"}</h2><p style={{ marginTop: 0, whiteSpace: "pre-wrap", color: "var(--senuke-muted)", fontSize: 17, lineHeight: 1.9 }}>{plainText(props.body)}</p></div></SectionShell>;
...
export function createSenukePuckConfig(theme: Theme = {}, mediaAssets: VisualMediaAsset[] = [], chrome?: WebsiteChrome): Config<any> {
  ...
  for (const definition of SENUKE_COMPONENT_REGISTRY_V1.components.filter((item) => item.lifecycleStatus === "active")) {
    ...
    for (const [fieldName, fieldDefinition] of Object.entries(definition.fields)) fields[fieldName] = registryField(fieldName, fieldDefinition, mediaAssets);
    components[definition.componentId] = {
      label: friendlyName(definition.componentId),
      fields,
      defaultProps: {
        variant: definition.variants[0],
        ...Object.fromEntries(Object.entries(definition.fields).map(([name, field]) => [name, field.type === "object_list" || field.type === "string_list" ? [] : field.type === "boolean" ? false : field.type === "number" ? 0 : ""])),
      },
...
export function puckToWebsiteComponents(data: Data<Record<string, PuckProps>>): WebsiteComponentInstance[] {
  ...
  for (const [index, item] of data.content.entries()) {
    ...
    const instance: WebsiteComponentInstance = {
