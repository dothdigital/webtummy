...
const additionalQuestionsSchema = z.preprocess(
  (value) => Array.isArray(value) ? value.slice(0, 5) : value,
  z.array(z.string().trim().min(1).max(500)).max(5),
).default([]);
export const aiIntakeOutputSchema: z.ZodTypeAny = z.object({ suggestions: z.record(suggestionSchema).superRefine((suggestions, ctx) => { for (const field of aiIntakeSuggestionFields) if (!(field in suggestions)) ctx.addIssue({ code: "custom", message: `Missing ${field}` }); }), additionalQuestions: additionalQuestionsSchema });
...
` : "";
      const generated = await centralAiJson({
        system: businessDiscovery
          ? "You are SEnuke AI - AI Growth Operating System's Business Discovery advisor, powered by the research model. First understand and structure everything the user supplied; then ask only one genuinely unresolved, decision-changing question."
          : "You are SEnuke AI - AI Growth Operating System's conversational project intake specialist. Understand natural language, ask one useful question at a time, and return reviewable structured project data. Never claim that suggested keywords have search-volume validation.",
      ...
      if (missingMandatory.length) {
        ...
        if (businessDiscovery && nextMandatory === "websiteUrl") {
          ...
          const correction = await centralAiJson({
            system: "You are SEnuke AI - AI Growth Operating System's Business Discovery advisor. Correct one incomplete follow-up question using the real project evidence. Do not use generic templates.",
...
AI keyword directions: ${JSON.stringify(semanticKeywordSuggestions).slice(0, 10_000)}`,
        ...
        const nextQuestion = businessDiscovery
          ...
          : fallbackQuestion;
        if (businessDiscovery && !nextQuestion) throw Object.assign(new Error("SEnuke AI - AI Growth Operating System could not prepare a relevant Business Discovery question. Please retry this response."), { code: "ai_output_invalid", statusCode: 502, publicMessage: true });
        output = {
          ...output,
...
${JSON.stringify(evidence).slice(0, 100_000)}`
          ...
          generated = await centralAiJson<ProjectLaunchProposal>({
            system: attempt === 0
              ? "You are SEnuke AI - AI Growth Operating System's senior business, market, search, website, and growth research strategist. Produce a practical, evidence-grounded launch proposal for human approval. The response contract is mandatory."
              : "You are SEnuke AI - AI Growth Operating System's structured-output repair specialist. Correct schema and type errors while preserving verified facts and useful research. Return exactly the requested object and nothing else.",
            prompt: activePrompt,
            model,
            timeoutMs: 120_000,
