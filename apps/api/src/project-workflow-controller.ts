...
export function resolveProjectWorkflow(snapshot: WorkflowEvidenceSnapshot): ProjectWorkflowControllerView {
  ...
  if (!snapshot.discoveryComplete) {
    ...
    nextBestAction = { title: strategyStale ? `Strategy v${snapshot.latestStrategyVersion || 1} was completed and now needs regeneration` : snapshot.latestStrategy ? "Review and approve Unified Strategy" : "Generate Unified Strategy", reason: strategyStale ? "The existing Strategy remains in version history, but newer evidence was collected after it was generated." : "Required intelligence is complete, so AI can now make evidence-backed decisions.", expectedResult: "One approved plan controlling Website, SEO, Local, Citations, Authority, Lead Magnets, Social, Growth, and Publishing.", action: action(strategyStale ? "Regenerate Strategy" : snapshot.latestStrategy ? "Review Strategy" : "Generate Strategy", `/strategy?${projectQuery}`, strategyStale || !snapshot.latestStrategy ? "generate" : "approve"), aiWill: [aiRoles.strategy.suggestion, aiRoles.strategy.implementation], userWill: snapshot.latestStrategy && !strategyStale ? aiRoles.approval.humanRole : aiRoles.strategy.humanRole, confidence: overallConfidence, explainability: strategyStale ? "The current Strategy was completed before newer evidence. Regeneration creates a new reviewable version and preserves the previous version in history." : "All applicable intelligence requirements are complete, so Strategy is the next governed decision layer." };
...
export async function getProjectWorkflowController(projectId: string): Promise<ProjectWorkflowControllerView | null> {
  const project = await prisma.project.findUnique({
    ...
    include: {
      ...
      nextBestActions: { where: { status: { in: ["proposed", "recommended", "selected"] } }, orderBy: [{ selectedAt: "desc" }, { priorityScore: "desc" }, { createdAt: "desc" }], take: 10, select: { id: true, title: true, recommendation: true, reasoningSummary: true, expectedImpact: true, confidence: true, route: true, evidenceJson: true, status: true } },
