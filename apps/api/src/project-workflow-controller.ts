...
export function resolveProjectApplicability(input: { projectType: string; websiteStatus: string; hasWebsite: boolean; websiteLaunched?: boolean; targetMarketCount?: number; contextText: string }) {
  ...
  return {
    ...
    requiredModules: WORKFLOW_MODULE_CAPABILITIES.filter((capability) => {
      ...
      if (capability.key === "ecommerce_intelligence") return input.projectType === "ecommerce";
...
export function resolveProjectWorkflow(snapshot: WorkflowEvidenceSnapshot): ProjectWorkflowControllerView {
  ...
  const ecommerceBlockedBy = snapshot.existingWebsite && !snapshot.siteAnalysisComplete ? "Complete Website Intelligence for the public store first." : null;
  const ecommerceStatus = moduleStatus({ required: snapshot.ecommerceApplicable, complete: snapshot.ecommerceAnalysisComplete, inProgress: snapshot.ecommerceAnalysisInProgress, failed: snapshot.ecommerceAnalysisFailed, blockedBy: ecommerceBlockedBy });
  push({ key: "ecommerce_intelligence", label: "Ecommerce Intelligence", description: snapshot.preLaunchWebsite ? "Confirmed products, categories, buying journeys, catalogue requirements, market demand, merchandising, checkout, and measurement planning before launch." : "Public products, collections, store structure, product search intent, merchandising, content, internal linking, and evidence limitations.", status: ecommerceStatus, required: snapshot.ecommerceApplicable, weight: 12, reason: statusReason(ecommerceStatus, snapshot.preLaunchWebsite ? "Pre-launch catalogue and buying-journey intelligence is available to Strategy." : "Public product and collection intelligence is available to Strategy.", snapshot.preLaunchWebsite ? "Research the confirmed product catalogue, market demand, buying journey, and measurement requirements before Strategy." : "Analyze the public catalog and approve the product or collection priorities that should influence Strategy.", ecommerceBlockedBy ?? undefined), evidenceAt: snapshot.ecommerceEvidenceAt?.toISOString() ?? null, action: snapshot.ecommerceApplicable ? action(snapshot.ecommerceAnalysisComplete ? "Review Ecommerce Intelligence" : snapshot.preLaunchWebsite ? "Start Ecommerce Research" : "Analyze public store", `/ecommerce-intelligence?${projectQuery}`, snapshot.ecommerceAnalysisComplete ? "review" : "generate") : null, ai: { ...aiRoles.intelligence, implementation: snapshot.preLaunchWebsite ? "AI uses confirmed Business Brain products and completed market evidence to plan the catalogue and buying journey without inventing live-store performance." : "AI classifies crawl-visible products and collections, compares them with search demand, labels inferred merchandising ideas, and never invents private sales, margin, inventory, or conversion data." } });
  ...
  // An approved downstream Strategy is durable evidence that its required
  // pre-Strategy intelligence was completed and reviewed at that time. Older
  // projects may not have rows in every newer module table; never send them
  // backward after Strategy, channel-plan, or Website Development approval.
  const intelligenceGovernanceSatisfied = intelligenceReady || strategyApproved;
  ...
  const findingsReviewed = Boolean(snapshot.findingsReviewed || strategyApproved || (intelligenceReady && snapshot.criticalEvidenceIssueCount === 0));
  ...
  // A completed external publication proves that the project already passed
  ...
  const stages: WorkflowStage[] = [
    ...
    { key: "business_brain_approval", label: "Business Brain Review and Approval", description: "Approve the exact Business Brain version used by downstream work.", status: businessBrainGovernanceSatisfied ? "approved" : snapshot.discoveryComplete ? "ready" : "blocked", reason: snapshot.publishingComplete ? "The published release preserves the approved Business Brain; newer facts are optional review." : snapshot.businessBrainApproved ? "The current Business Brain version is approved." : snapshot.discoveryComplete ? "Review and approve the current Business Brain before readiness checks." : "Complete Intake first.", action: action(businessBrainGovernanceSatisfied ? "Review Business Brain" : "Approve Business Brain", `/guided-projects/${snapshot.projectId}`, businessBrainGovernanceSatisfied ? "review" : "approve"), ai: aiRoles.approval },
    { key: "readiness_check", label: "Readiness Check", description: "Confirm required project facts and situation details before Opportunity Discovery.", status: readinessGovernanceSatisfied ? "complete" : businessBrainGovernanceSatisfied ? "ready" : "blocked", reason: snapshot.publishingComplete ? "The published release confirms the completed readiness path." : snapshot.readinessComplete ? "The current Business Brain passed readiness." : businessBrainGovernanceSatisfied ? "Confirm readiness for Opportunity Discovery." : "Approve the Business Brain first.", action: action(readinessGovernanceSatisfied ? "Review Readiness" : "Confirm Readiness", `/guided-projects/${snapshot.projectId}`, readinessGovernanceSatisfied ? "review" : "approve"), ai: aiRoles.setup },
    ...
    { key: "required_intelligence", label: "Required Intelligence", description: "Collect only evidence applicable to this project.", status: intelligenceGovernanceSatisfied ? "complete" : requiredIntelligence.some((item) => item.status === "in_progress") ? "in_progress" : snapshot.selectedOpportunity ? "ready" : "blocked", reason: strategyApproved && !intelligenceReady ? "The approved Strategy preserves completion of its pre-Strategy intelligence; newer module records are optional history." : intelligenceReady ? "Every required module is Complete or Not Applicable." : `${requiredIntelligence.filter((item) => !["complete", "approved", "not_applicable"].includes(item.status)).length} required intelligence area(s) remain.`, action: intelligenceGovernanceSatisfied ? null : intelligence.find((item) => item.required && !["complete", "approved", "not_applicable"].includes(item.status))?.action ?? action("Review intelligence", `/seo-growth?${projectQuery}`, "review"), ai: aiRoles.intelligence, modules: intelligence },
  ...
  if (snapshot.publishingComplete) {
  ...
  } else if (!businessBrainGovernanceSatisfied) {
    state = "discovery";
  ...
  } else if (!readinessGovernanceSatisfied) {
    state = "discovery";
  ...
  } else if (!strategyApproved && !intelligenceReady && incompleteIntelligence) {
    state = "intelligence_collection";
    ...
    snapshot.websitePlanApproved
    && snapshot.websiteDevelopmentStarted
    && !snapshot.publishingComplete
    && !snapshot.preparedChangesAwaitingApproval
    && (snapshot.openExecutionTasks > 0 || snapshot.publishingStarted)
  ) {
    state = "execution";
    nextBestAction = { title: "Continue Website Development", reason: snapshot.publishingStarted ? "The website release is prepared and publishing setup is in progress. Resume the exact project website where you left it." : "Website Development has started. Resume the saved build, review, approval, readiness, or publishing step instead of returning to completed planning work.", expectedResult: "The current project website resumed at its saved Website Development state.", action: action("Open Website Development", `/site-architect?${projectQuery}`, "implement"), aiWill: ["Load the latest saved website build and background-worker results", "Preserve completed pages, approvals, media, quality checks, and release state", "Keep the live website unchanged until explicit publishing"], userWill: "Continue from the saved Website Development step.", confidence: overallConfidence, explainability: "An active website build is the current resumable work. Older planning gaps and unrelated execution-plan administration must not interrupt it." };
...
export async function getProjectWorkflowController(projectId: string): Promise<ProjectWorkflowControllerView | null> {
  ...
  const ecommerceApplicable = project.projectType === "ecommerce";
