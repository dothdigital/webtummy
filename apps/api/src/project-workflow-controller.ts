...
export function resolveProjectWorkflow(snapshot: WorkflowEvidenceSnapshot): ProjectWorkflowControllerView {
  ...
  const continuousGrowthReady = Boolean(businessBrainGovernanceSatisfied && readinessGovernanceSatisfied && intelligenceReady && findingsReviewed && strategyApproved && growthBlueprintApproved && snapshot.executionPlanApproved && snapshot.completedExecutionTasks > 0 && snapshot.measurementComplete && reportingLearningComplete && trackingReady && snapshot.nextBestActionExists);
