...
function nextActionFor(state: CanonicalExecutionState, contract: ModuleContract, hasSnapshot: boolean) {
  if (!hasSnapshot || state === "DRAFT") return { key: "prepare", label: "Prepare execution", reason: "Resolve authority, context, outputs, safeguards, approval, destination, and measurement before work starts." };
  if (state === "BLOCKED" || state === "STALE") return { key: "resolve", label: "Resolve blocker", reason: "Reconcile the task with its current Strategy, evidence, dependencies, permission, or destination capability." };
  if (["READY", "RESERVED"].includes(state)) return { key: "start", label: `Start ${contract.label} work`, reason: "The governed Work Package is ready for its module workflow." };
  if (["RUNNING", "ESTIMATING"].includes(state)) return { key: "continue", label: "Continue execution", reason: "Resume from the latest safe checkpoint and preserve completed work." };
  if (["VALIDATING", "NEEDS_REVIEW", "CHANGES_REQUESTED"].includes(state)) return { key: "review", label: "Review work", reason: "Validate the exact version, resolve findings, and approve only the immutable result." };
  ...
  if (["QUEUED", "EXECUTING_EXTERNAL", "VERIFYING"].includes(state)) return { key: "verify", label: "Verify external result", reason: "Provider acknowledgement is not success; confirm the remote state and exact version." };
  if (["ACTIVE", "MEASURING"].includes(state)) return { key: "measure", label: "View results", reason: "Collect the approved outcome signals before Growth Intelligence selects another action." };
  if (state === "FAILED") return { key: "retry", label: "Recover execution", reason: "Preserve valid drafts and retry from the latest safe checkpoint." };
