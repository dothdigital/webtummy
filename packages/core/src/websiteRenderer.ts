...
type BlogPageIdentity = { pageType: string; name?: string; slug?: string; seo?: { canonicalUrl?: string } };

const blogSourcePath = (page: BlogPageIdentity) => {
  try { return new URL(page.seo?.canonicalUrl || page.slug || "/", "https://website.invalid").pathname.replace(/\/+$/, ""); }
  catch { return ""; }
};

export const isWebsiteBlogSectionPage = (page: BlogPageIdentity) =>
  page.pageType.trim().toLowerCase() === "blog_section"
  || (page.pageType.trim().toLowerCase() === "blog" && /^(?:blog|insights|resources)$/i.test(page.name?.trim() || ""))
  || /^\/blog(?:\.(?:html?|php|aspx?))?$/i.test(blogSourcePath(page))
  || /^\/blog(?:-html)?\/?$/i.test(page.slug || "");

export const isWebsiteBlogArticlePage = (page: BlogPageIdentity) =>
  /^(?:blog[_-]?article|blog[_-]?post|post|article|news)$/i.test(page.pageType.trim())
  || (/^(?:supporting|service|blog)$/i.test(page.pageType.trim()) && (/^\/blog\/[^/]+/i.test(blogSourcePath(page)) || /^\/blog\/[^/]+/i.test(page.slug || "")));

/**
 * Keep article slugs simple in Site Architect while publishing them beneath
 * the approved Blog Section in static output.
 */
export function websitePagePublicationPath(model: Pick<WebsiteModel, "pages">, page: WebsitePageModel) {
  const originalPath = normalizedPath(page.slug);
  if (!isWebsiteBlogArticlePage(page)) return originalPath;
  const parent = page.parentPageId
    ? model.pages.find((candidate) => candidate.pageId === page.parentPageId && isWebsiteBlogSectionPage(candidate))
    : undefined;
  const blogSection = parent || model.pages.find(isWebsiteBlogSectionPage);
  if (!blogSection) return originalPath;
  const blogPath = normalizedPath(blogSection.slug);
  if (originalPath.startsWith(blogPath) && originalPath !== blogPath) return originalPath;
  const articleSlug = originalPath.replace(/^\/|\/$/g, "").split("/").filter(Boolean).at(-1) || page.pageId;
  return normalizedPath(`${blogPath}${articleSlug}`);
}
...
const internalLinkHref = (model: WebsiteModel, targetPageId: string, options: WebsiteRenderOptions) => {
  ...
  if (!target) return "";
  const path = websitePagePublicationPath(model, target);
...
export function renderWebsiteComponentHtml(
  ...
  switch (component.componentId) {
    ...
    case "conversion.contact_form":
      {
        const submissionUrl = propString(component, "submissionUrl");
        const providerEmbedHtml = propString(component, "providerEmbedHtml");
        const formId = propString(component, "formId");
        const sectionId = formId.replace(/[^a-z0-9_-]+/gi, "-").replace(/^-|-$/g, "");
        if (providerEmbedHtml) {
          return `<section${sectionId ? ` id="${escapeHtml(sectionId)}"` : ""} class="senuke-component senuke-contact-form senuke-provider-contact-form ${alignmentClass}"><div><h2>${heading}</h2><p>${escapeHtml(propString(component, "introduction"))}</p></div><iframe class="senuke-provider-form-embed" title="${escapeHtml(`${propString(component, "heading") || "Registration"} form`)}" sandbox="allow-forms allow-scripts allow-popups" referrerpolicy="no-referrer" loading="lazy" srcdoc="${escapeHtml(providerEmbedHtml)}"></iframe></section>`;
        }
        if (options.formShortcode && !submissionUrl) {
...
export function renderWebsitePageBodyHtml(
  ...
  const internalUrlMap = {
    ...Object.fromEntries(model.pages.flatMap((candidate) => {
      const originalPath = normalizedPath(candidate.slug);
      const publicationPath = websitePagePublicationPath(model, candidate);
      return [[originalPath, publicationPath], [publicationPath, publicationPath]];
    })),
  ...
  const formHtml = isContactPage && form && !hasRegisteredForm
    ...
    : "";
  const blogIndexHtml = isWebsiteBlogSectionPage(page)
    ? renderWebsiteBlogIndexHtml(model, page, componentOptions)
    : "";
  if (blogIndexHtml) {
...
const renderWebsiteBlogIndexHtml = (
  model: WebsiteModel,
  blogSection: WebsitePageModel,
  options: WebsiteRenderOptions,
) => {
  const hero = flattenWebsiteComponents(blogSection.sections).find((section) => section.componentId === "hero.local_service");
  const heading = String(hero?.props.headline || blogSection.name || "Blog").trim();
  const introduction = String(hero?.props.summary || "Read the latest articles, insights, and practical guidance.").replace(/\s+/g, " ").trim().slice(0, 240);
  const sections = model.pages.filter(isWebsiteBlogSectionPage);
  const articles = model.pages.filter((candidate) => isWebsiteBlogArticlePage(candidate)
    && (candidate.parentPageId === blogSection.pageId || (!candidate.parentPageId && sections.length === 1)));
  // Keep the authored hero and its approved image instead of replacing it with
  // an unrelated text-only archive heading. The grid supplies the archive body.
  const heroHtml = hero ? renderWebsiteComponentHtml(hero, options) : "";
  const archiveHeading = hero ? "" : `<div class="senuke-blog-index-heading"><p class="senuke-eyebrow">Blog</p><h1>${escapeHtml(heading)}</h1><p>${escapeHtml(introduction)}</p></div>`;
  if (!articles.length) {
    return `${heroHtml}<section id="articles" class="senuke-blog-index">${archiveHeading}<p>New articles are being prepared.</p></section>`;
  }
  return `${heroHtml}<section id="articles" class="senuke-blog-index">${archiveHeading}<div class="senuke-blog-grid">${articles.map((article) => {
    const publicationPath = websitePagePublicationPath(model, article);
    const href = options.internalUrlMap?.[publicationPath] || publicationPath;
    const heroAssetId = String(flattenWebsiteComponents(article.sections).find((section) => section.componentId === "hero.local_service")?.props.imageAssetId || "");
    ...
    const articleWords = flattenWebsiteComponents(article.sections).filter(section => section.componentId === "content.rich_text").map(section => propString(section, "body")).join(" ").trim().split(/\s+/).filter(Boolean).length;
    const readingMinutes = Math.max(1, Math.ceil(articleWords / 200));
    return `<article class="senuke-blog-card">${imageUrl && renderableImageUrl(imageUrl) ? `<a class="senuke-blog-card-image" href="${escapeHtml(href)}"><img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(asset?.altText || article.name)}" width="640" height="360" loading="lazy" decoding="async"></a>` : ""}<div><p class="senuke-blog-topic">${readingMinutes} min read</p><h3><a href="${escapeHtml(href)}">${escapeHtml(article.name)}</a></h3><p>${escapeHtml(article.seo.metaDescription)}</p><a class="senuke-blog-read-more" href="${escapeHtml(href)}">Read article <span aria-hidden="true">→</span></a></div></article>`;
  }).join("")}</div></section>`;
};
...
export function renderWebsitePageWordPressBlocks(
  ...
  const blockPayloads = page.sections.map((component, index) => {
    ...
    // Lead-magnet forms have their own verified SEnuke submission endpoint.
    // Keep their managed form markup in WordPress instead of replacing it with
    // the website-wide contact-form shortcode, which would send the registrant
    // to the wrong destination and skip lead-magnet delivery.
    const hasDedicatedSubmission = component.componentId === "conversion.contact_form"
      && Boolean(propString(component, "submissionUrl") || propString(component, "providerEmbedHtml"));
    const blockMarkup = hasDedicatedSubmission
      ? rendered
      : serializeWordPressComponentBlock(component, options, index === secondFoldIndex);
...
export function renderWebsitePageDocument(
  ...
  const publicationPath = websitePagePublicationPath(model, page);
  const structuredPublicationPath = publicationPath !== normalizedPath(page.slug);
  const canonical = production && /^https:\/\//i.test(page.seo.canonicalUrl) && !structuredPublicationPath
    ? page.seo.canonicalUrl
    : production && options.baseUrl
      ? `${options.baseUrl.replace(/\/+$/, "")}${structuredPublicationPath ? publicationPath : normalizedPath(page.seo.canonicalUrl || page.slug)}`
      : structuredPublicationPath ? publicationPath : normalizedPath(page.seo.canonicalUrl || page.slug);
  ...
  const disclaimerText = model.identity?.disclaimerText?.trim() || "";
...
document.querySelectorAll(".senuke-testimonials").forEach(function(section){
  ...
  var slides=slider.querySelectorAll(".senuke-testimonial-card");
  var previous=controls.querySelector('[data-testimonial-direction="-1"]');
  ...
  controls.querySelectorAll("button").forEach(function(button){button.addEventListener("click",function(){
    ...
    slider.scrollTo({left:index*slider.clientWidth,behavior:window.matchMedia("(prefers-reduced-motion: reduce)").matches?"instant":"smooth"});
  });});
...
.senuke-second-fold{text-align:inherit}
.senuke-second-fold>h2,.senuke-second-fold>div>h2{max-width:30ch;margin:0 0 1.15rem;text-align:inherit;text-wrap:balance}
.senuke-second-fold>p,.senuke-second-fold>div>p{max-width:var(--senuke-reading-max,72ch);margin:.8rem 0 0;text-align:inherit}
.senuke-rich-text{width:min(var(--senuke-layout-max,1120px),calc(100% - var(--senuke-layout-inset,2rem)));padding-block:clamp(3rem,5vw,4.5rem);text-align:left}
.senuke-rich-text h2{max-width:30ch;margin:0 0 1.15rem;font-size:clamp(1.75rem,3vw,2.35rem);letter-spacing:-.02em;text-wrap:balance}
.senuke-rich-text p{max-width:var(--senuke-reading-max,72ch);margin:.8rem 0 0;color:var(--senuke-muted);font-size:1rem;line-height:1.75}
...
.senuke-hero-image{width:100%;height:auto;max-height:620px;border-radius:1.25rem;object-fit:cover}
...
.senuke-button{display:inline-block;width:fit-content;max-width:100%;white-space:normal;overflow-wrap:anywhere;border:0;border-radius:.75rem;background:var(--senuke-primary);color:#fff!important;font-weight:800;padding:.85rem 1.2rem;text-decoration:none}
...
.senuke-provider-form-embed{display:block;width:100%;min-height:640px;border:0;border-radius:1.25rem;background:var(--senuke-surface);box-shadow:0 20px 60px rgba(15,23,42,.1)}
...
.senuke-footer-disclaimer{position:relative;z-index:1;width:min(var(--senuke-layout-max,1120px),calc(100% - var(--senuke-layout-inset,2rem)));margin:0 auto 1rem;padding:1rem 1.25rem;border-top:1px solid rgba(255,255,255,.12);color:rgba(255,255,255,.7);font-size:.78rem;line-height:1.7}
...
body{background:var(--senuke-surface)}
...
.senuke-hero{position:relative;isolation:isolate;width:100%;max-width:none;padding-inline:max(var(--senuke-layout-gutter,1rem),calc((100% - var(--senuke-layout-max,1120px))/2));background:linear-gradient(120deg,color-mix(in srgb,var(--senuke-primary) 6%,var(--senuke-surface)),var(--senuke-surface))}
.senuke-hero>div{min-width:0}.senuke-hero h1{margin:1rem 0 1.5rem;font-size:clamp(2.35rem,4.5vw,4.25rem);font-weight:750;line-height:1.08;letter-spacing:-.045em;text-wrap:balance}.senuke-hero-image{aspect-ratio:4/3;box-shadow:0 18px 48px rgba(15,23,42,.1)}
.senuke-hero .senuke-lead,.senuke-rich-text>p,.senuke-internal-link-introduction,.senuke-internal-link-closing{color:color-mix(in srgb,var(--senuke-text) 78%,transparent)}
.senuke-card,.senuke-faq details,.senuke-contact-form form,.senuke-site-header li ul,.senuke-mobile-menu-panel{color:#0f172a}
.senuke-card p,.senuke-faq details p,.senuke-contact-form form p{color:#475569}
.senuke-contact-form input,.senuke-contact-form textarea{color:var(--senuke-text)}
.senuke-rich-text{display:block;width:min(var(--senuke-layout-max,1120px),calc(100% - var(--senuke-layout-inset,2rem)));padding-block:clamp(3rem,5vw,4.5rem);text-align:left}.senuke-rich-text h2{max-width:30ch;margin:0 0 1.15rem;font-size:clamp(1.75rem,3vw,2.35rem);letter-spacing:-.02em;text-wrap:balance}.senuke-rich-text p{max-width:var(--senuke-reading-max,72ch);margin:.8rem 0 0;color:var(--senuke-muted);font-size:1rem;line-height:1.75}
...
.senuke-layout-column>.senuke-rich-text p{text-align:inherit}
.senuke-component{scroll-margin-top:7rem}
.senuke-hero.senuke-document{display:block;min-height:0;padding-block:3.5rem 2.75rem;background:color-mix(in srgb,var(--senuke-primary) 4%,var(--senuke-surface));border-bottom:1px solid color-mix(in srgb,var(--senuke-muted) 16%,transparent)}
.senuke-document>div{max-width:780px;margin-inline:auto}.senuke-document h1{max-width:100%;font-size:clamp(2rem,4vw,3.25rem)}.senuke-document .senuke-lead{max-width:68ch;font-size:1.05rem}
:is(main,.entry-content):has(.senuke-document) .senuke-rich-text{width:min(780px,calc(100% - 3rem));padding-block:1.75rem}
:is(main,.entry-content):has(.senuke-document) .senuke-rich-text h2{max-width:100%;font-size:1.4rem;line-height:1.35}
:is(main,.entry-content):has(.senuke-document) .senuke-rich-text p{max-width:100%;font-size:1rem;line-height:1.85;color:var(--senuke-text)}
:is(main,.entry-content):has(.senuke-document) .senuke-rich-text+.senuke-rich-text{border-top:1px solid color-mix(in srgb,var(--senuke-muted) 12%,transparent)}
.senuke-rich-text.senuke-second-fold{box-shadow:none;border-radius:0;margin-top:0}
...
export function createStaticWebsiteFiles(
  ...
  const pageFiles = model.pages.map((page) => {
    const path = websitePagePublicationPath(model, page);
    ...
    const internalUrlMap = Object.fromEntries(model.pages.flatMap((targetPage) => {
      const originalPath = normalizedPath(targetPage.slug);
      const targetPath = websitePagePublicationPath(model, targetPage);
      const targetFile = targetPath === "/"
        ? "index.html"
        : `${targetPath.replace(/^\/|\/$/g, "")}/index.html`;
      const relativeTarget = `${relativeRoot}${targetFile}`;
      return [[originalPath, relativeTarget], [targetPath, relativeTarget]];
    }));
    internalUrlMap["/sitemap.xml"] = `${relativeRoot}sitemap.xml`;
    internalUrlMap["/rss.xml"] = `${relativeRoot}rss.xml`;
  ...
  const rssFile: WebsiteRenderFile[] = blogSection && blogArticles.length ? [{
    ...
    content: `<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>${escapeHtml(`${model.identity?.businessName || "Website"} articles`)}</title><link>${escapeHtml(options.baseUrl ? `${options.baseUrl.replace(/\/+$/, "")}${websitePagePublicationPath(model, blogSection)}` : websitePagePublicationPath(model, blogSection))}</link><description>${escapeHtml(blogSection.seo.metaDescription)}</description>${blogArticles.map((article) => {
      const path = websitePagePublicationPath(model, article);
      const link = options.baseUrl ? `${options.baseUrl.replace(/\/+$/, "")}${path}` : path;
      return `<item><title>${escapeHtml(article.name)}</title><link>${escapeHtml(link)}</link><guid>${escapeHtml(link)}</guid><description>${escapeHtml(article.seo.metaDescription)}</description></item>`;
    }).join("")}</channel></rss>`,
  }] : [];
  const manifest = {
    ...
    pages: model.pages.map((page) => ({ pageId: page.pageId, name: page.name, path: websitePagePublicationPath(model, page), pageType: page.pageType, parentPageId: page.parentPageId ?? null })),
  };
  return [
    ...pageFiles,
    ...mediaFiles,
